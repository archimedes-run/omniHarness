...
if [ -n "$OMNI_HARNESS_CONFIG_PATH" ] && [ -f "$OMNI_HARNESS_CONFIG_PATH" ]; then
    CONFIG="$OMNI_HARNESS_CONFIG_PATH"
...
MIGRATIONS = {
    1: {
        'description': 'Rename src.* module paths to omniharness.*',
        'replacements': [
            ('src.community.', 'omniharness.community.'),
            ('src.sandbox.', 'omniharness.sandbox.'),
            ('src.models.', 'omniharness.models.'),
            ('src.tools.', 'omniharness.tools.'),
        ],
    },

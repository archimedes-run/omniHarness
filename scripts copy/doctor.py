#!/usr/bin/env python3
"""OmniHarness Health Check (make doctor).
...
def check_llm_auth(config_path: Path) -> list[CheckResult]:
    ...
    try:
        ...
        for model in data.get("models", []):
            ...
            if use == "omniharness.models.openai_codex_provider:CodexChatModel":
            ...
            if use == "omniharness.models.claude_provider:ClaudeChatModel":
                credential_paths = [
...
def main() -> int:
    ...
    print()
    print(bold("OmniHarness Health Check"))
    print("═" * 40)
    ...
    if total_fails == 0 and total_warns == 0:
        ...
        print(f"Run {cyan('make dev')} to start OmniHarness")
    elif total_fails == 0:

#!/usr/bin/env bash
#
# serve.sh — Unified OmniHarness service launcher
#
# Usage:
...
stop_all() {
    ...
    _kill_port 8001
    _kill_port 3000
    ./scripts/cleanup-containers.sh omni-harness-sandbox 2>/dev/null || true
...
if $DEV_MODE && ! $DAEMON_MODE; then
    GATEWAY_EXTRA_FLAGS="--reload --reload-include='*.yaml' --reload-include='.env' --reload-exclude='*.pyc' --reload-exclude='__pycache__' --reload-exclude='sandbox/' --reload-exclude='.omni-harness/'"
else
    GATEWAY_EXTRA_FLAGS=""
fi
...
if ! { \
        [ -n "$OMNI_HARNESS_CONFIG_PATH" ] && [ -f "$OMNI_HARNESS_CONFIG_PATH" ] || \
        [ -f backend/config.yaml ] || \
        [ -f config.yaml ]; \
    }; then
    echo "✗ No OmniHarness config file found."
...
echo ""
echo "=========================================="
echo "  ✓ OmniHarness is running!  [$MODE_LABEL]"
echo "=========================================="
echo ""

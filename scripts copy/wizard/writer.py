...
def _default_tools() -> list[dict[str, Any]]:
    return [
        {"name": "image_search", "use": "omniharness.community.image_search.tools:image_search_tool", "group": "web", "max_results": 5},
        {"name": "ls", "use": "omniharness.sandbox.tools:ls_tool", "group": "file:read"},
        {"name": "read_file", "use": "omniharness.sandbox.tools:read_file_tool", "group": "file:read"},
        {"name": "glob", "use": "omniharness.sandbox.tools:glob_tool", "group": "file:read"},
        {"name": "grep", "use": "omniharness.sandbox.tools:grep_tool", "group": "file:read"},
        {"name": "write_file", "use": "omniharness.sandbox.tools:write_file_tool", "group": "file:write"},
        {"name": "str_replace", "use": "omniharness.sandbox.tools:str_replace_tool", "group": "file:write"},
        {"name": "bash", "use": "omniharness.sandbox.tools:bash_tool", "group": "bash"},
    ]
...
def _build_tools(
    *,
    ...
    if include_bash_tool:
        tools.append({"name": "bash", "use": "omniharness.sandbox.tools:bash_tool", "group": "bash"})
...
def build_minimal_config(
    *,
    ...
    sandbox_use: str = "omniharness.sandbox.local:LocalSandboxProvider",
    ...
    if sandbox_use == "omniharness.sandbox.local:LocalSandboxProvider":

# Contributing to OmniHarness Backend

Thank you for your interest in contributing to OmniHarness! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/omni-harness.git
   cd omni-harness
   ```
...
```python
import pytest
from omniharness.models.factory import create_chat_model
...
1. Create tool in `packages/harness/omniharness/tools/builtins/` or `packages/harness/omniharness/community/`:

```python
# packages/harness/omniharness/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: omniharness.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/omniharness/agents/middlewares/`:

```python
# packages/harness/omniharness/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/omniharness/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/omniharness/config/app_config.py` with new fields
...
Thank you for contributing to OmniHarness!

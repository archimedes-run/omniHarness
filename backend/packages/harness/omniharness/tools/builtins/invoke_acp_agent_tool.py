...
def _build_mcp_servers() -> dict[str, dict[str, Any]]:
    """Build ACP ``mcpServers`` config from OmniHarness's enabled MCP servers."""
...
def _build_acp_mcp_servers() -> list[dict[str, Any]]:
    ...
    The ACP client expects a list of server objects, while OmniHarness's MCP helper
        ...
        try:
            ...
            async with spawn_agent_process(client, cmd, *args, env=agent_env, cwd=physical_cwd) as (conn, proc):
                ...
                await conn.initialize(
                    ...
                    client_info=Implementation(name="omniharness", title="OmniHarness", version="0.1.0"),
                )

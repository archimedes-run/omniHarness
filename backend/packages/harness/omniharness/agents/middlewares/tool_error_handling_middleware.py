...
def _build_runtime_middlewares(
    *,
    ...
    from omniharness.agents.middlewares.llm_error_handling_middleware import LLMErrorHandlingMiddleware
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "omniharness"
    ...
    from omniharness.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware

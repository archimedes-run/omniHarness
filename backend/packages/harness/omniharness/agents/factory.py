"""Pure-argument factory for OmniHarness agents.

``create_omniharness_agent`` accepts plain Python arguments — no YAML files, no
...
from omniharness.agents.middlewares.dangling_tool_call_middleware import DanglingToolCallMiddleware
from omniharness.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware
from omniharness.agents.thread_state import ThreadState
from omniharness.tools.builtins import ask_clarification_tool
...
def create_omniharness_agent(
    ...
    *,
    ...
    """Create a OmniHarness agent from plain Python arguments.
    ...
    *,
    ...
    if feat.sandbox is not False:
        if isinstance(feat.sandbox, AgentMiddleware):
            ...
            from omniharness.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
            from omniharness.agents.middlewares.uploads_middleware import UploadsMiddleware
    ...
    if feat.vision is not False:
        ...
        if feat.sandbox is not False:
            from omniharness.tools.builtins import view_image_tool
    ...
    if feat.subagent is not False:
        ...
        from omniharness.tools.builtins import task_tool

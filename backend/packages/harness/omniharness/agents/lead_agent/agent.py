...
from omniharness.agents.memory.summarization_hook import memory_flush_hook
from omniharness.agents.middlewares.clarification_middleware import ClarificationMiddleware
from omniharness.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
from omniharness.agents.middlewares.memory_middleware import MemoryMiddleware
from omniharness.agents.middlewares.subagent_limit_middleware import SubagentLimitMiddleware
from omniharness.agents.middlewares.summarization_middleware import BeforeSummarizationHook, OmniHarnessSummarizationMiddleware
from omniharness.agents.middlewares.title_middleware import TitleMiddleware
from omniharness.agents.middlewares.todo_middleware import TodoMiddleware
from omniharness.agents.middlewares.token_usage_middleware import TokenUsageMiddleware
from omniharness.agents.middlewares.tool_error_handling_middleware import build_lead_runtime_middlewares
from omniharness.agents.middlewares.view_image_middleware import ViewImageMiddleware
from omniharness.agents.thread_state import ThreadState
from omniharness.config.agents_config import load_agent_config, validate_agent_name
from omniharness.config.app_config import AppConfig, get_app_config
...
from omniharness.skills.types import Skill
...
def _create_summarization_middleware(*, app_config: AppConfig | None = None) -> OmniHarnessSummarizationMiddleware | None:
    ...
    # the sole entry point for OmniHarnessSummarizationMiddleware, and the runtime
    # config is not expected to change after startup.
    skills_container_path = resolved_app_config.skills.container_path or "/mnt/skills"

    return OmniHarnessSummarizationMiddleware(
        **kwargs,
...
def _create_todo_list_middleware(is_plan_mode: bool) -> TodoMiddleware | None:
    ...
    # Custom prompts matching OmniHarness's style
    system_prompt = """
<todo_list_system>
...
def _build_middlewares(
    ...
    *,
    ...
    if resolved_app_config.tool_search.enabled:
        from omniharness.agents.middlewares.deferred_tool_filter_middleware import DeferredToolFilterMiddleware
...
def _load_enabled_skills_for_tool_policy(available_skills: set[str] | None, *, app_config: AppConfig) -> list[Skill]:
    try:
        from omniharness.agents.lead_agent.prompt import get_enabled_skills_for_config

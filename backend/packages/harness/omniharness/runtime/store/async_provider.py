...
Usage (e.g. FastAPI lifespan)::
    ...
    from omniharness.runtime.store import make_store
...
from omniharness.runtime.store.provider import POSTGRES_CONN_REQUIRED, POSTGRES_STORE_INSTALL, SQLITE_STORE_INSTALL, ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
async def _async_store(config) -> AsyncIterator[BaseStore]:
    ...
    The ``config`` argument is a :class:`omniharness.config.checkpointer_config.CheckpointerConfig`
...
async def make_store(app_config: AppConfig | None = None) -> AsyncIterator[BaseStore]:
    ...
    :func:`omniharness.runtime.checkpointer.async_provider.make_checkpointer` so

"""OmniHarnessClient — Embedded Python client for OmniHarness agent system.

Provides direct programmatic access to OmniHarness's agent capabilities
...
from omniharness.agents.lead_agent.agent import _build_middlewares
from omniharness.agents.lead_agent.prompt import apply_prompt_template
from omniharness.agents.thread_state import ThreadState
from omniharness.config.agents_config import AGENT_NAME_PATTERN
from omniharness.config.app_config import get_app_config, reload_app_config
from omniharness.config.extensions_config import ExtensionsConfig, SkillStateConfig, get_extensions_config, reload_extensions_config
...
from omniharness.uploads.manager import (
...
class OmniHarnessClient:
    """Embedded Python client for OmniHarness agent system.
    ...
    def _ai_tool_calls_event(msg_id: str | None, tool_calls, additional_kwargs: dict[str, Any] | None = None) -> "StreamEvent":
        ...
        data: dict[str, Any] = {
            ...
            "content": "",
            "id": msg_id,
            "tool_calls": OmniHarnessClient._serialize_tool_calls(tool_calls),
        }
        if additional_kwargs:
            data["additional_kwargs"] = additional_kwargs
        return StreamEvent(type="messages-tuple", data=data)
    ...
    def _tool_message_event(msg: ToolMessage) -> "StreamEvent":
        ...
        return StreamEvent(
            ...
            data={
                "type": "tool",
                "content": OmniHarnessClient._extract_text(msg.content),
                "name": msg.name,
    ...
    def _serialize_message(msg) -> dict:
        ...
        if isinstance(msg, AIMessage):
            ...
            if msg.tool_calls:
                d["tool_calls"] = OmniHarnessClient._serialize_tool_calls(msg.tool_calls)
            if getattr(msg, "usage_metadata", None):
                d["usage_metadata"] = msg.usage_metadata
            if additional_kwargs := OmniHarnessClient._serialize_additional_kwargs(msg):
    ...
    def list_threads(self, limit: int = 10) -> dict:
        ...
        checkpointer = self._checkpointer
        if checkpointer is None:
            from omniharness.runtime.checkpointer.provider import get_checkpointer
    ...
    def stream(
        ...
        * ``StreamBridge`` is an asyncio-queue decoupling producers from
        ...
        So ``OmniHarnessClient.stream()`` is a parallel, sync, in-process
    ...
    def get_memory(self) -> dict:
        ...
        """
        from omniharness.agents.memory.updater import get_memory_data
    ...
    def import_memory(self, memory_data: dict) -> dict:
        ...
        from omniharness.agents.memory.updater import import_memory_data
    ...
    def update_mcp_config(self, mcp_servers: dict[str, dict]) -> dict:
        ...
        if config_path is None:
            raise FileNotFoundError("Cannot locate extensions_config.json. Set OMNI_HARNESS_EXTENSIONS_CONFIG_PATH or ensure it exists in the project root.")
    ...
    def reload_memory(self) -> dict:
        ...
        """
        from omniharness.agents.memory.updater import reload_memory_data

        return reload_memory_data(user_id=get_effective_user_id())

    def clear_memory(self) -> dict:
        """Clear all persisted memory data."""
        from omniharness.agents.memory.updater import clear_memory_data
    ...
    def create_memory_fact(self, content: str, category: str = "context", confidence: float = 0.5) -> dict:
        ...
        from omniharness.agents.memory.updater import create_memory_fact

        return create_memory_fact(content=content, category=category, confidence=confidence)

    def delete_memory_fact(self, fact_id: str) -> dict:
        """Delete a single fact from memory by fact id."""
        from omniharness.agents.memory.updater import delete_memory_fact
    ...
    def update_memory_fact(
        ...
        from omniharness.agents.memory.updater import update_memory_fact
    ...
    def get_memory_config(self) -> dict:
        ...
        Returns:
            Memory config dict.
        """
        from omniharness.config.memory_config import get_memory_config
    ...
    def upload_files(self, thread_id: str, files: list[str | Path]) -> dict:
        ...
        """
        from omniharness.utils.file_conversion import CONVERTIBLE_EXTENSIONS, convert_file_to_markdown
    ...
    def delete_upload(self, thread_id: str, filename: str) -> dict:
        ...
        """
        from omniharness.utils.file_conversion import CONVERTIBLE_EXTENSIONS

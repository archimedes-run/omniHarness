...
def project_root() -> Path:
    ...
    if env_root := os.getenv("OMNI_HARNESS_PROJECT_ROOT"):
        root = Path(env_root).resolve()
        if not root.exists():
            raise ValueError(f"OMNI_HARNESS_PROJECT_ROOT is set to '{env_root}', but the resolved path '{root}' does not exist.")
        if not root.is_dir():
            raise ValueError(f"OMNI_HARNESS_PROJECT_ROOT is set to '{env_root}', but the resolved path '{root}' is not a directory.")
        return root
    return Path.cwd().resolve()


def runtime_home() -> Path:
    """Return the writable OmniHarness state directory."""
    if env_home := os.getenv("OMNI_HARNESS_HOME"):
        return Path(env_home).resolve()
    return project_root() / ".omni-harness"

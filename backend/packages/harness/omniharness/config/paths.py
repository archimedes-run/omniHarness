...
from omniharness.config.runtime_paths import runtime_home
...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable OmniHarness state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.omni-harness``).  Using
...
class Paths:
    """
    Centralized path configuration for OmniHarness application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. OMNI_HARNESS_HOME environment variable
        3. Caller project fallback: `{project_root}/.omni-harness`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set OMNI_HARNESS_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("OMNI_HARNESS_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("OMNI_HARNESS_HOME"):

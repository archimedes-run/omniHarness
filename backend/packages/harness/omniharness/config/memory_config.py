...
class MemoryConfig(BaseModel):
    ...
    storage_path: str = Field(
        ...
        description=(
            ...
            "Note: if you previously set this to `.omni-harness/memory.json`, "
            "the file will now be resolved as `{base_dir}/.omni-harness/memory.json`; "
            "migrate existing data or use an absolute path to preserve the old location."
        ),
    )
    storage_class: str = Field(
        default="omniharness.agents.memory.storage.FileMemoryStorage",

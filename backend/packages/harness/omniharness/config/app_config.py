...
from omniharness.config.acp_config import ACPAgentConfig, load_acp_config_from_dict
from omniharness.config.agents_api_config import AgentsApiConfig, load_agents_api_config_from_dict
from omniharness.config.checkpointer_config import CheckpointerConfig, load_checkpointer_config_from_dict
...
from omniharness.config.skill_evolution_config import SkillEvolutionConfig
from omniharness.config.skills_config import SkillsConfig
from omniharness.config.stream_bridge_config import StreamBridgeConfig, load_stream_bridge_config_from_dict
from omniharness.config.subagents_config import SubagentsAppConfig, load_subagents_config_from_dict
from omniharness.config.summarization_config import SummarizationConfig, load_summarization_config_from_dict
from omniharness.config.title_config import TitleConfig, load_title_config_from_dict
...
from omniharness.config.trigger_engine_config import TriggerEngineConfig
...
def apply_logging_level(name: str | None) -> None:
    """Resolve *name* to a logging level and apply it to the ``omniharness``/``app`` logger hierarchies.

    Only the ``omniharness`` and ``app`` logger levels are changed so that
    ...
    for logger_name in ("omniharness", "app"):
...
class AppConfig(BaseModel):
    """Config for the OmniHarness application"""

    log_level: str = Field(default="info", description="Logging level for omniharness and app modules (debug/info/warning/error); third-party libraries are not affected")
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `OMNI_HARNESS_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("OMNI_HARNESS_CONFIG_PATH"):
            path = Path(os.getenv("OMNI_HARNESS_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `OMNI_HARNESS_CONFIG_PATH` not found at {path}")
            return path
        else:
    ...
    def _apply_singleton_configs(cls, config: Self, acp_agents: dict[str, ACPAgentConfig]) -> None:
        from omniharness.config.checkpointer_config import get_checkpointer_config
        ...
        if previous_checkpointer_config != config.checkpointer:
            ...
            from omniharness.runtime.checkpointer import reset_checkpointer
            from omniharness.runtime.store import reset_store
...
_current_app_config: ContextVar[AppConfig | None] = ContextVar("omniharness_current_app_config", default=None)
_current_app_config_stack: ContextVar[tuple[AppConfig | None, ...]] = ContextVar("omniharness_current_app_config_stack", default=())
...
def get_app_config() -> AppConfig:
    """Get the OmniHarness config instance.

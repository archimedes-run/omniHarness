...
OmniHarness now supports Apple Container as the preferred container runtime on macOS, with automatic fallback to Docker.

## Overview

Starting with this version, OmniHarness automatically detects and uses Apple Container on macOS when available, falling back to Docker when:
...
The implementation is in `backend/packages/harness/omniharness/community/aio_sandbox/aio_sandbox_provider.py`:
...
```
INFO:omniharness.community.aio_sandbox.aio_sandbox_provider:Detected Apple Container: container version 0.1.0
INFO:omniharness.community.aio_sandbox.aio_sandbox_provider:Starting sandbox container using container: ...
```

Or for Docker:
```
INFO:omniharness.community.aio_sandbox.aio_sandbox_provider:Apple Container not available, falling back to Docker
INFO:omniharness.community.aio_sandbox.aio_sandbox_provider:Starting sandbox container using docker: ...
```
...
sandbox:
  ...
  image: ghcr.io/archimedes-run/omni-harness-sandbox:latest  # Default image
```
...
container image pull ghcr.io/archimedes-run/omni-harness-sandbox:latest

# Using Docker
docker pull ghcr.io/archimedes-run/omni-harness-sandbox:latest
```
...
**Usage:**
```bash
# Clean up all OmniHarness sandbox containers
./scripts/cleanup-containers.sh omni-harness-sandbox

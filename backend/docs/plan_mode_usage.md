...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in OmniHarness 2.0.
...
### Agent Module
- **Location**: `packages/harness/omniharness/agents/lead_agent/agent.py`
...
OmniHarness uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall OmniHarness prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with OmniHarness's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/omni-harness/backend/packages/harness/omniharness/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom OmniHarness-style prompts**
...
- Custom prompts emphasize the same principles as OmniHarness's main system prompt (clarity, action-oriented, critical rules)

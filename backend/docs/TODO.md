...
- [x] Replace `time.sleep(5)` with `asyncio.sleep()` in `packages/harness/omniharness/tools/builtins/task_tool.py` (subagent polling)
...
- [ ] Replace `subprocess.run()` with `asyncio.create_subprocess_shell()` in `packages/harness/omniharness/sandbox/local/local_sandbox.py`

# [RFC] 在 OmniHarness 中增加 `grep` 与 `glob` 文件搜索工具

## Summary

我认为这个方向是对的，而且值得做。

如果 OmniHarness 想更接近 Claude Code 这类 coding agent 的实际工作流，仅有 `ls` / `read_file` / `write_file` / `str_replace` 还不够。模型在进入修改前，通常还需要两类能力：
...
结论：OmniHarness 现在缺的不是“再多一个 shell 命令”，而是**文件系统检索层**。
...
- 不把它做成“任意磁盘搜索”，仍然只允许在 OmniHarness 已授权的路径内执行
...
5. **对 `tool_search` 友好**
   当 OmniHarness 持续扩展工具集时，`grep` / `glob` 会成为非常高频的基础工具，值得保留为 built-in，而不是让模型总是退回通用 bash。
...
- `backend/packages/harness/omniharness/sandbox/tools.py`
...
- 在非 local sandbox 场景，优先也通过 OmniHarness 自己控制的路径访问层实现
...
这会让 OmniHarness 在代码探索体验上持续落后，也削弱无 bash 或受限 bash 场景下的能力。
...
MCP 可以是补充，但 `glob` / `grep` 作为 OmniHarness 的基础 coding tool，最好仍然是 built-in，这样才能在默认安装中稳定可用。
...
tools:
  - name: glob
    group: file:read
    use: omniharness.sandbox.tools:glob_tool

  - name: grep
    group: file:read
    use: omniharness.sandbox.tools:grep_tool
```
...
如果按这个方向做，它会明显提升 OmniHarness 在 coding / repo exploration 场景下的可用性，而且风险可控。

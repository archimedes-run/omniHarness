# RFC: `create_omniharness_agent` — 纯参数的 SDK 工厂 API
...
**6 处隐式 I/O** — 全部依赖文件系统。如果你想把 `omniharness-harness` 当 Python 库嵌入自己的应用，你必须准备 `config.yaml` + `extensions_config.json` + skills 目录。这对 SDK 用户是不可接受的。

### 对比

| | `langchain.create_agent` | `make_lead_agent` | `OmniHarnessClient`（增强后） |
|---|---|---|---|
...
| 适合谁 | 写 LangChain 的人 | 内部使用 | **所有 OmniHarness 用户** |
...
```
    ┌──────────────────────┐
    │   OmniHarnessClient     │  ← 唯一公开 API（chat/stream + 管理）
    └──────────┬───────────┘
    ┌──────────▼───────────┐
    │   make_lead_agent    │  ← 内部：配置驱动工厂
    └──────────┬───────────┘
    ┌──────────▼───────────┐
    │  create_omniharness_agent   │  ← 内部：纯参数工厂
    └──────────┬───────────┘
    ┌──────────▼───────────┐
...
`OmniHarnessClient` 是唯一公开 API。`create_omniharness_agent` 和 `make_lead_agent` 都是内部实现。

用户通过 `OmniHarnessClient` 三个参数控制行为：
...
- **向前兼容** — 现有 `OmniHarnessClient()` 无参构造行为不变
- **harness 边界合规** — 不 import `app.*`（`test_harness_boundary.py` 强制）

## 3. API 设计

### 3.1 `OmniHarnessClient` — 唯一公开 API

在现有构造函数上增加三个可选参数：

```python
from omniharness.client import OmniHarnessClient
from omniharness.agents.features import RuntimeFeatures
...
client = OmniHarnessClient(
    ...
    config={
        ...
        "sandbox": {"use": "omniharness.sandbox.local:LocalSandboxProvider"},
    },
...
# 用法 2：只改参数，不换实现
client = OmniHarnessClient(config={"memory": {"max_facts": 50}})

# 用法 3：替换 middleware 实现
client = OmniHarnessClient(features=RuntimeFeatures(auto_title=MyTitleMiddleware()))

# 用法 4：添加自定义 middleware
client = OmniHarnessClient(extra_middleware=[MyAuditMiddleware()])

# 用法 5：纯 SDK（无 config.yaml）
client = OmniHarnessClient(config={
    "models": [{"name": "gpt-4o", "use": "langchain_openai:ChatOpenAI", ...}],
    "tools": [{"name": "bash", "use": "omniharness.sandbox.tools:bash_tool", "group": "bash"}],
    "memory": {"enabled": True},
})
```
...
### 3.2 `create_omniharness_agent` — 内部工厂（不公开）

```python
def create_omniharness_agent(
    model: BaseChatModel,
...
client = OmniHarnessClient(features=RuntimeFeatures(memory=MyMemoryMiddleware()))
...
```python
from omniharness.agents import Next, Prev
...
client = OmniHarnessClient(config={
    "models": [{"name": "gpt-4o", "use": "langchain_openai:ChatOpenAI", "model": "gpt-4o", "api_key": "sk-..."}],
    "tools": [
        {"name": "bash", "group": "bash", "use": "omniharness.sandbox.tools:bash_tool"},
        {"name": "web_search", "group": "web", "use": "omniharness.community.tavily.tools:web_search_tool"},
    ],
...
```python
from omniharness.agents import Next, Prev
from omniharness.sandbox.middleware import SandboxMiddleware
from omniharness.agents.middlewares.clarification_middleware import ClarificationMiddleware
...
```
Phase 1（当前 PR #1203）:
  ✓ 新增 create_omniharness_agent + RuntimeFeatures（内部 API）
  ✓ 不改 OmniHarnessClient 和 make_lead_agent
  ✗ middleware 内部仍读 config（已知限制）

Phase 2（#1380）:
  - OmniHarnessClient 构造函数增加可选参数（model, tools, features, system_prompt）
  - Options 参数覆盖 config（MemoryOptions, TitleOptions 等）
  - @Next/@Prev 装饰器
  - 补缺失 middleware（Guardrail, TokenUsage, DeferredToolFilter）
  - make_lead_agent 改为薄壳调 create_omniharness_agent

Phase 3:
  - SDK 文档和示例
  - omniharness.client 稳定 API
```
...
| 问题 | 决议 | 理由 |
|------|------|------|
| 公开 API | `OmniHarnessClient` 唯一入口 | 自顶向下，先改现有 API 再抽底层 |
| create_omniharness_agent | 内部实现，不公开 | 用户不需要接触 CompiledStateGraph |

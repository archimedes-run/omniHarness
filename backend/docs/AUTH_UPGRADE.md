...
OmniHarness 内置了认证模块。本文档面向从无认证版本升级的用户。
...
| 文件 | 内容 |
|------|------|
| `.omni-harness/users.db` | SQLite 用户数据库（密码哈希、角色） |
...
- **Docker 部署**：完全兼容，`.omni-harness/users.db` 需持久化卷挂载
- **IM 渠道**（Feishu/Slack/Telegram）：通过 LangGraph SDK 通信，不经过认证层
- **OmniHarnessClient**（嵌入式）：不经过 HTTP，不受认证影响

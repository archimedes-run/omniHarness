...
本文档解释 OmniHarness 是如何把 LangGraph agent 的事件流端到端送到两类消费者（HTTP 客户端、嵌入式 Python 调用方）的：两条路径为什么**必须**并存、它们各自的契约是什么、以及设计里那些 non-obvious 的不变式。

---

## TL;DR

- OmniHarness 有**两条并行**的流式路径：**Gateway 路径**（async / HTTP SSE / JSON 序列化）服务浏览器和 IM 渠道；**OmniHarnessClient 路径**（sync / in-process / 原生 LangChain 对象）服务 Jupyter、脚本、测试。它们**无法合并**——消费者模型不同。
...
| 维度 | Gateway 路径 | OmniHarnessClient 路径 |
|---|---|---|
| 入口 | FastAPI `/runs/stream` endpoint | `OmniHarnessClient.stream(message)` |
| 触发层 | `runtime/runs/worker.py::run_agent` | `packages/harness/omniharness/client.py::OmniHarnessClient.stream` |
...
### 为什么不能让 OmniHarnessClient 复用 Gateway

曾经考虑过三种复用方案，都被否决：

1. **让 `client.stream()` 变成 `async def client.astream()`**  
   breaking change。用户用不上的 `async for` / `asyncio.run()` 要硬塞进 Jupyter notebook 和同步脚本。OmniHarnessClient 的一大卖点（"把 agent 当普通函数调用"）直接消失。
...
**后果**：`OmniHarnessClient.stream()` 直接调 `agent.stream()`（Graph 层），所以必须传 `"messages"`。`app/channels/manager.py` 通过 `langgraph-sdk` 走 HTTP SDK，所以传 `"messages-tuple"`。**这两个字符串不能互相替代**，也不能抽成"一个共享常量"——它们是不同协议层的 type alias，共享只会让某一层说不是它母语的话。
...
## OmniHarnessClient 路径：sync + in-process

```mermaid
sequenceDiagram
    participant User as Python caller
    participant Client as OmniHarnessClient.stream
...
这个语义和 LangChain 的 `fs2 Stream` 风格一致：**上游发增量，下游负责累加**。Gateway 路径里前端 `useStream` React hook 自己维护累加器；OmniHarnessClient 路径里 `chat()` 方法替调用者做累加。

### `OmniHarnessClient.chat()` 的 O(n) 累加器
...
`OmniHarnessClient.stream()` 在一次调用生命周期内维护三个 `set[str]`：
...
sequenceDiagram
    ...
    participant C as OmniHarnessClient
...
本文档的直接起因是 archimedes-run/omni-harness#1969：`OmniHarnessClient.stream()` 原本只订阅 `["values", "custom"]`，**漏了 `"messages"`**。结果 `client.stream("hello")` 等价于一次性返回，视觉上和 `chat()` 没区别。

这类 bug 有三个结构性原因：

1. **多协议层命名**：`messages` / `messages-tuple` / HTTP SSE `messages` 是同一概念的三个名字。在其中一层出错不会在另外两层报错。
2. **多消费者模型**：Gateway 和 OmniHarnessClient 是两套独立实现，**没有单一的"订阅哪些 mode"的 single source of truth**。前者订阅对了不代表后者也订阅对了。
...
| 关心什么 | 看这里 |
|---|---|
| OmniHarnessClient 嵌入式流 | `packages/harness/omniharness/client.py::OmniHarnessClient.stream` |
| `chat()` 的 delta 累加器 | `packages/harness/omniharness/client.py::OmniHarnessClient.chat` |
| Gateway async 流 | `packages/harness/omniharness/runtime/runs/worker.py::run_agent` |
| HTTP SSE 帧输出 | `app/gateway/services.py::sse_consumer` / `format_sse` |
| 序列化到 wire 格式 | `packages/harness/omniharness/runtime/serialization.py` |
| LangGraph mode 命名翻译 | `packages/harness/omniharness/runtime/runs/worker.py:117-121` |

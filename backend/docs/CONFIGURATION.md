...
This guide explains how to configure OmniHarness for your environment.
...
- Claude Code OAuth (`omniharness.models.claude_provider:ClaudeChatModel`)
- Codex CLI (`omniharness.models.openai_codex_provider:CodexChatModel`)
...
- On macOS, OmniHarness does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Standard `langchain_openai:ChatOpenAI` silently drops `thought_signature` when serialising messages.  Use `omniharness.models.patched_openai:PatchedChatOpenAI` instead — it re-injects the tool-call signatures (sourced from `AIMessage.additional_kwargs["tool_calls"]`) into every outgoing payload:

```yaml
models:
  - name: gemini-2.5-pro-thinking
    display_name: Gemini 2.5 Pro (Thinking)
    use: omniharness.models.patched_openai:PatchedChatOpenAI
...
tools:
  - name: web_search
    group: web
    use: omniharness.community.tavily.tools:web_search_tool
    max_results: 5
...
OmniHarness supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:

**Local Execution** (runs sandbox code directly on the host machine):
```yaml
sandbox:
   use: omniharness.sandbox.local:LocalSandboxProvider # Local execution
...
```yaml
sandbox:
   use: omniharness.community.aio_sandbox:AioSandboxProvider # Docker-based sandbox
```
...
When using Docker development (`make docker-start`), OmniHarness starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
`allow_host_bash` is intentionally `false` by default. OmniHarness's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.
...
When you configure `sandbox.mounts`, OmniHarness exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.

For bare-metal Docker sandbox runs that use localhost, OmniHarness binds the sandbox HTTP port to `127.0.0.1` by default so it is not exposed on every host interface. Docker-outside-of-Docker deployments that connect through `host.docker.internal` keep the broad legacy bind for compatibility. Set `OMNI_HARNESS_SANDBOX_BIND_HOST` explicitly if your deployment needs a different bind address.
...
**How Skills Work**:
- Skills are stored in `omni-harness/skills/{public,custom}/`
...
2. Restart the OmniHarness service to apply changes

## Environment Variables

OmniHarness supports environment variable substitution using the `$` prefix:
...
- `OMNI_HARNESS_PROJECT_ROOT` - Project root for relative runtime paths
- `OMNI_HARNESS_CONFIG_PATH` - Custom config file path
- `OMNI_HARNESS_EXTENSIONS_CONFIG_PATH` - Custom extensions config file path
- `OMNI_HARNESS_HOME` - Runtime state directory (defaults to `.omni-harness` under the project root)
- `OMNI_HARNESS_SKILLS_PATH` - Skills directory when `skills.path` is omitted
...
The configuration file should be placed in the **project root directory** (`omni-harness/config.yaml`). Set `OMNI_HARNESS_PROJECT_ROOT` when the process may start from another working directory, or set `OMNI_HARNESS_CONFIG_PATH` to point at a specific file.

## Configuration Priority

OmniHarness searches for configuration in this order:

1. Path specified in code via `config_path` argument
2. Path from `OMNI_HARNESS_CONFIG_PATH` environment variable
3. `config.yaml` under `OMNI_HARNESS_PROJECT_ROOT`, or under the current working directory when `OMNI_HARNESS_PROJECT_ROOT` is unset
4. Legacy backend/repository-root locations for monorepo compatibility

## Best Practices

1. **Place `config.yaml` in project root** - Set `OMNI_HARNESS_PROJECT_ROOT` if the runtime starts elsewhere
...
- Ensure `config.yaml` exists in the **project root** directory (`omni-harness/config.yaml`)
- If the runtime starts outside the project root, set `OMNI_HARNESS_PROJECT_ROOT`
- Alternatively, set `OMNI_HARNESS_CONFIG_PATH` environment variable to custom location
...
### "Skills not loading"
- Check that `omni-harness/skills/` directory exists
- Verify skills have valid `SKILL.md` files
- Check `skills.path` or `OMNI_HARNESS_SKILLS_PATH` if using a custom path

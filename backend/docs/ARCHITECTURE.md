...
This document provides a comprehensive overview of the OmniHarness backend architecture.
...
**Entry Point**: `packages/harness/omniharness/agents/lead_agent/agent.py:make_lead_agent`
...
```json
{
  "agent": {
    "type": "agent",
    "path": "omniharness.agents:make_lead_agent"
  }
}
```
...
- `threads.py` - `/api/threads/{id}` - Local OmniHarness thread data cleanup after LangGraph deletion
- `artifacts.py` - `/api/threads/{id}/artifacts` - Artifact serving
- `suggestions.py` - `/api/threads/{id}/suggestions` - Follow-up suggestion generation

The web conversation delete flow is now split across both backend surfaces: LangGraph handles `DELETE /api/langgraph/threads/{thread_id}` for thread state, then the Gateway `threads.py` router removes OmniHarness-managed filesystem data via `Paths.delete_thread_dir()`.
...
│  (packages/harness/omniharness/sandbox/local.py) │              │  (packages/harness/omniharness/community/)       │
│                         │              │                         │
...
|-------------|---------------|
| `/mnt/user-data/workspace` | `backend/.omni-harness/threads/{thread_id}/user-data/workspace` |
| `/mnt/user-data/uploads` | `backend/.omni-harness/threads/{thread_id}/user-data/uploads` |
| `/mnt/user-data/outputs` | `backend/.omni-harness/threads/{thread_id}/user-data/outputs` |
| `/mnt/skills` | `omni-harness/skills/` |
...
│  (packages/harness/omniharness/tools/)       │  │  (config.yaml)      │  │  (extensions.json)  │
├─────────────────────┤  ├─────────────────────┤  ├─────────────────────┤
...
└─────────────────────┘  └─────────────────────┘  └─────────────────────┘
           ...
           └───────────────────────┴───────────────────────┘
                      ...
                      ┌─────────────────────────┐
                      │   get_available_tools() │
                      │   (packages/harness/omniharness/tools/__init__)  │
                      └─────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Model Factory                                   │
│                     (packages/harness/omniharness/models/factory.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          MCP Integration                                 │
│                        (packages/harness/omniharness/mcp/manager.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Skills System                                   │
│                       (packages/harness/omniharness/skills/loader.py)                             │
└─────────────────────────────────────────────────────────────────────────┘
...
2. Gateway receives file
   - Validates file
   - Stores in .omni-harness/threads/{thread_id}/user-data/uploads/
...
3. Returns response
   {
     "files": [{
       "filename": "doc.pdf",
       "path": ".omni-harness/.../uploads/doc.pdf",
...
3. Gateway removes local OmniHarness-managed files
   - Deletes .omni-harness/threads/{thread_id}/ recursively

...
`create_omniharness_agent` 通过 `RuntimeFeatures` 组装的完整 middleware 链（默认全开时）：
...
主 agent **14 个** middleware（`make_lead_agent`），subagent **4 个**（ThreadData、Sandbox、Guardrail、ToolErrorHandling）。`create_omniharness_agent` Phase 1 实现 **13 个**（Guardrail 仅支持自定义实例，无内置默认）。

...
|---|---|---|---|
| TC-DOCKER-01 | `users.db` volume persistence | Verify the `OMNI_HARNESS_HOME` bind mount survives container restart | needs `docker compose up` |
...
| TC-DOCKER-05 | Admin credentials surfacing | **Updated post-simplify** — was "log scrape", now "0600 credential file in `OMNI_HARNESS_HOME`". The file-based behavior is already validated by TC-1.1 + TC-UPG-13 on sg_dev (non-Docker), so the only Docker-specific gap is verifying the volume mount carries the file out to the host | needs container + host volume |
...
# Optional: pin OMNI_HARNESS_HOME to a stable host path
echo "OMNI_HARNESS_HOME=$HOME/omni-harness-data" >> .env
```

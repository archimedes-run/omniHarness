...
Gateway (`app/gateway/routers/skills.py`, `uploads.py`) and Client (`omniharness/client.py`) each independently implement the same business logic:
...
app.gateway.routers.uploads ──┤── calls ──→  omniharness.skills.installer
omniharness.client             ──┘              omniharness.uploads.manager
```

- Shared modules live in the harness layer (`omniharness.*`), pure business logic, no FastAPI dependency
...
### 3.1 `omniharness.skills.installer`
...
### 3.2 `omniharness.uploads.manager`
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `OmniHarnessClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `OmniHarnessClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.

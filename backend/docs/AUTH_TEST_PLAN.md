...
```bash
# 清除已有数据
rm -f backend/.omni-harness/users.db
...
curl -s -X POST $BASE/api/v1/auth/login/local \
  -d "username=admin@omniharness.dev&password=<控制台密码>" \
  -c cookies.txt | jq .
```
...
**预期：** `{"id": "...", "email": "admin@omniharness.dev", "system_role": "admin", "needs_setup": true}`
...
```bash
# 检查数据库
sqlite3 backend/.omni-harness/users.db "SELECT email, password_hash FROM users LIMIT 3;"
```
...
**预期：**
- [ ] 控制台输出 admin 邮箱（`admin@omniharness.dev`）和随机密码
- [ ] 无报错，正常启动
...
```bash
ls -la backend/.omni-harness/users.db
```
...
```bash
sqlite3 backend/.omni-harness/users.db "PRAGMA journal_mode;"
```
...
# 运维终于登录
curl -s -X POST $BASE/api/v1/auth/login/local \
  -d "username=admin@omniharness.dev&password=<P0或P1>" \
...
# 检查 admin 数量
sqlite3 backend/.omni-harness/users.db \
...
> - `OMNI_HARNESS_HOME` 挂载到宿主机目录（持久化 `users.db`）
...
# 检查宿主机上的 users.db
ls -la ${OMNI_HARNESS_HOME:-backend/.omni-harness}/users.db
sqlite3 ${OMNI_HARNESS_HOME:-backend/.omni-harness}/users.db \
  "SELECT email FROM users WHERE email='docker-test@example.com';"
```

**预期：** users.db 在宿主机 `OMNI_HARNESS_HOME` 目录中，查询可见刚注册的用户。
...
docker logs omni-harness-gateway 2>&1 | grep -E "ChannelManager|channel" | head -10
```
...
```bash
# 凭证文件写在挂载到宿主机的 OMNI_HARNESS_HOME 下
ls -la ${OMNI_HARNESS_HOME:-backend/.omni-harness}/admin_initial_credentials.txt
# 预期文件权限: -rw------- (0600)

cat ${OMNI_HARNESS_HOME:-backend/.omni-harness}/admin_initial_credentials.txt
# 预期内容: email + password 行

# 容器日志只输出文件路径，不输出密码本身
docker logs omni-harness-gateway 2>&1 | grep -E "Credentials written to|Admin account"
# 预期看到: "Credentials written to: /...../admin_initial_credentials.txt (mode 0600)"

# 反向验证: 日志里 NEVER 出现明文密码
docker logs omni-harness-gateway 2>&1 | grep -iE "Password: .{15,}" && echo "FAIL: leaked" || echo "OK: not leaked"
```

**预期：**
- 凭证文件存在于 `OMNI_HARNESS_HOME` 下，权限 `0600`
...
# 确认 langgraph 容器不存在
docker ps --filter name=omni-harness-langgraph --format '{{.Names}}' | wc -l
# 预期: 0
...
curl -s -X POST $BASE/api/v1/auth/login/local \
  -d "username=admin@omniharness.dev&password=<日志密码>" \
  -c cookies.txt -w "\nHTTP %{http_code}"
# 预期: 200
```

...
from omniharness.agents.memory.storage import (
    FileMemoryStorage,
    MemoryStorage,
    create_empty_memory,
    get_memory_storage,
...
class TestFileMemoryStorage:
    ...
    def test_get_memory_file_path_global(self, tmp_path):
        ...
        with patch("omniharness.agents.memory.storage.get_paths", side_effect=mock_get_paths):
            with patch("omniharness.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_path="")):
...
class TestGetMemoryStorage:
    ...
    def reset_storage_instance(self):
        ...
        import omniharness.agents.memory.storage as storage_mod
    ...
    def test_returns_file_memory_storage_by_default(self):
        ...
        with patch("omniharness.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="omniharness.agents.memory.storage.FileMemoryStorage")):
    ...
    def test_falls_back_to_file_memory_storage_on_error(self):
        ...
        with patch("omniharness.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="non.existent.StorageClass")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
    ...
    def test_get_memory_storage_invalid_class_fallback(self):
        ...
        with patch("omniharness.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="os.path.join")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
    ...
    def test_get_memory_storage_non_subclass_fallback(self):
        ...
        with patch("omniharness.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="builtins.dict")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)

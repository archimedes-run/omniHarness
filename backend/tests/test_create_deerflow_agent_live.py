"""Live integration tests for create_omniharness_agent.
...
def test_minimal_agent_responds():
    """create_omniharness_agent(model) produces a graph that returns a response."""
    from omniharness.agents.factory import create_omniharness_agent

    model = _make_model()
    graph = create_omniharness_agent(model, features=None, middleware=[])
...
def test_agent_with_custom_tool():
    ...
    model = _make_model()
    graph = create_omniharness_agent(model, tools=[add], middleware=[])
...
def test_features_mode_middleware_chain():
    ...
    graph = create_omniharness_agent(model, features=feat)

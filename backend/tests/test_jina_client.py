...
import omniharness.community.jina_ai.jina_client as jina_client_module
from omniharness.community.jina_ai.jina_client import JinaClient
from omniharness.community.jina_ai.tools import web_fetch_tool
...
async def test_crawl_transient_failure_logs_without_traceback(jina_client, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="omniharness.community.jina_ai.jina_client"):
        result = await jina_client.crawl("https://example.com")

    jina_records = [r for r in caplog.records if r.name == "omniharness.community.jina_ai.jina_client"]
...
async def test_crawl_warns_once_when_api_key_missing(jina_client, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="omniharness.community.jina_ai.jina_client"):
        await jina_client.crawl("https://example.com")
        await jina_client.crawl("https://example.com")
...
async def test_web_fetch_tool_returns_error_on_crawl_failure(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.community.jina_ai.tools.get_app_config", lambda: mock_config)
...
async def test_web_fetch_tool_offloads_extraction_to_thread(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.community.jina_ai.tools.asyncio.to_thread", tracking_to_thread)
    result = await web_fetch_tool.ainvoke("https://example.com")

...
from omniharness.agents.memory.prompt import format_conversation_for_update
...
def test_apply_updates_skips_existing_duplicate_and_preserves_removals() -> None:
    ...
    with patch(
        "omniharness.agents.memory.updater.get_memory_config",
...
def test_apply_updates_skips_same_batch_duplicates_and_keeps_source_metadata() -> None:
    ...
    update_data = {
        "newFacts": [
            ...
            {"content": "User works on OmniHarness", "category": "context", "confidence": 0.87},
        ],
    }
...
def test_clear_memory_data_resets_all_sections() -> None:
    with patch("omniharness.agents.memory.updater._save_memory_to_file", return_value=True):
...
def test_delete_memory_fact_removes_only_matching_fact() -> None:
    ...
    with (
        patch("omniharness.agents.memory.updater.get_memory_data", return_value=current_memory),
        patch("omniharness.agents.memory.updater._save_memory_to_file", return_value=True),
    ):
...
def test_create_memory_fact_appends_manual_fact() -> None:
    with (
        patch("omniharness.agents.memory.updater.get_memory_data", return_value=_make_memory()),
        patch("omniharness.agents.memory.updater._save_memory_to_file", return_value=True),
    ):
...
def test_delete_memory_fact_raises_for_unknown_id() -> None:
    with patch("omniharness.agents.memory.updater.get_memory_data", return_value=_make_memory()):
        try:
...
def test_import_memory_data_saves_and_returns_imported_memory() -> None:
    imported_memory = _make_memory(
        facts=[
            {
                "id": "fact_import",
                "content": "User works on OmniHarness.",
                "category": "context",
                "confidence": 0.87,
    ...
    with patch("omniharness.agents.memory.updater.get_memory_storage", return_value=mock_storage):
...
def test_update_memory_fact_rejects_invalid_confidence() -> None:
    ...
    for confidence in (-0.1, 1.1, float("nan"), float("inf"), float("-inf")):
        with patch(
            "omniharness.agents.memory.updater.get_memory_data",
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_string_response_parses(self):
        ...
        with (
            ...
            patch("omniharness.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True)),
            patch("omniharness.agents.memory.updater.get_memory_data", return_value=_make_memory()),
            patch("omniharness.agents.memory.updater.get_memory_storage", return_value=MagicMock(save=MagicMock(return_value=True))),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "Hello"
    ...
    def test_sync_update_memory_returns_false_when_executor_down(self):
        ...
        with (
            patch(
                "omniharness.agents.memory.updater._SYNC_MEMORY_UPDATER_EXECUTOR.submit",
...
class TestFinalizeCacheIsolation:
    ...
    def test_deepcopy_prevents_cache_corruption_on_save_failure(self):
        ...
        with (
            ...
            patch("omniharness.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True, fact_confidence_threshold=0.7)),
            patch("omniharness.agents.memory.updater.get_memory_data", return_value=original_memory),
            patch("omniharness.agents.memory.updater.get_memory_storage", return_value=MagicMock(save=save_mock)),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "hello"
...
class TestUserIdForwarding:
    ...
    def test_sync_update_forwards_user_id_to_load_and_save(self):
        ...
        with (
            patch.object(updater, "_get_model", return_value=model),
            patch("omniharness.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True)),
            patch("omniharness.agents.memory.updater.get_memory_data", return_value=_make_memory()) as mock_load,
            patch("omniharness.agents.memory.updater.get_memory_storage", return_value=mock_storage),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "Hello"

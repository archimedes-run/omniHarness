"""End-to-end tests for OmniHarnessClient.
...
Only OMNI_HARNESS_HOME is redirected to tmp_path for filesystem isolation.
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - OMNI_HARNESS_HOME → tmp_path (all thread data lands in a temp dir)
    - OMNI_HARNESS_PROJECT_ROOT → repository root (shared skills/config assets
    ...
    monkeypatch.setenv("OMNI_HARNESS_HOME", str(tmp_path))
    monkeypatch.setenv(
        "OMNI_HARNESS_PROJECT_ROOT",
        str(Path(__file__).resolve().parents[2]),
    )
    monkeypatch.setattr("omniharness.config.paths._paths", None)
    monkeypatch.setattr("omniharness.sandbox.sandbox_provider._default_sandbox_provider", None)
    ...
    monkeypatch.setattr("omniharness.config.app_config._app_config", None)
    monkeypatch.setattr("omniharness.config.app_config._app_config_is_custom", False)
    config = _make_e2e_config()
    monkeypatch.setattr("omniharness.config.app_config._app_config", config)
    monkeypatch.setattr("omniharness.config.app_config._app_config_is_custom", True)
    monkeypatch.setattr("omniharness.client.get_app_config", lambda: config)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from omniharness.config.title_config import TitleConfig

    monkeypatch.setattr("omniharness.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from omniharness.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "omniharness.agents.middlewares.memory_middleware.get_memory_config",
        lambda: MemoryConfig(enabled=False),
    )

    # 5. Ensure summarization is off (default, but be explicit)
    from omniharness.config.summarization_config import SummarizationConfig

    monkeypatch.setattr("omniharness.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from omniharness.agents.lead_agent.agent import _build_middlewares as _original_build_middlewares
    from omniharness.agents.middlewares.title_middleware import TitleMiddleware
    ...
    monkeypatch.setattr("omniharness.client._build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A OmniHarnessClient wired to the isolated e2e_env."""
    return OmniHarnessClient(checkpointer=None, thinking_enabled=False)
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = OmniHarnessClient(checkpointer=None, plan_mode=False)
        ...
        c2 = OmniHarnessClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _allow_skill_security_scan(self, monkeypatch):
        ...
        monkeypatch.setattr("omniharness.skills.installer.scan_skill_content", _scan)
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "omniharness.skills.storage._default_skill_storage",
...
class TestConfigManagement:
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OMNI_HARNESS_EXTENSIONS_CONFIG_PATH", str(config_file))

        # Force reload so the singleton picks up our test file
        from omniharness.config.extensions_config import reload_extensions_config

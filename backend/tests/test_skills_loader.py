...
def test_get_skills_root_path_points_to_current_project_skills(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.delenv("OMNI_HARNESS_SKILLS_PATH", raising=False)
    monkeypatch.delenv("OMNI_HARNESS_PROJECT_ROOT", raising=False)
...
def test_get_skills_root_path_honors_env_override(tmp_path: Path, monkeypatch):
    """OMNI_HARNESS_SKILLS_PATH should override the caller project skills directory."""
    skills_root = tmp_path / "team-skills"
    monkeypatch.setenv("OMNI_HARNESS_SKILLS_PATH", str(skills_root))

...
from omniharness.agents.memory.queue import ConversationContext, MemoryUpdateQueue
...
def test_queue_add_preserves_existing_correction_flag_for_same_thread() -> None:
    ...
    with (
        patch("omniharness.agents.memory.queue.get_memory_config", return_value=_memory_config(enabled=True)),
...
def test_process_queue_forwards_correction_flag_to_updater() -> None:
    ...
    with patch("omniharness.agents.memory.updater.MemoryUpdater", return_value=mock_updater):
...
def test_flush_nowait_cancels_existing_timer_and_starts_immediate_timer() -> None:
    ...
    with patch("omniharness.agents.memory.queue.threading.Timer", return_value=created_timer) as timer_cls:
        queue.flush_nowait()
...
def test_add_nowait_cancels_existing_timer_and_starts_immediate_timer() -> None:
    ...
    with (
        patch("omniharness.agents.memory.queue.get_memory_config", return_value=_memory_config(enabled=True)),
        patch("omniharness.agents.memory.queue.threading.Timer", return_value=created_timer) as timer_cls,
    ):

...
class TestChannelManager:
    def test_get_client_includes_csrf_header_and_cookie(self):
        ...
        assert headers["X-OmniHarness-Internal-Token"]
    ...
    def test_fetch_gateway_includes_internal_auth_headers(self, monkeypatch):
        ...
        async def go():
            ...
            assert calls[0]["headers"]["X-OmniHarness-Internal-Token"]
    ...
    def test_each_topic_creates_new_thread(self):
        """Messages with distinct topic_ids should each create a new OmniHarness thread."""
        from app.channels.manager import ChannelManager
    ...
    def test_same_topic_reuses_thread(self):
        """Messages with the same topic_id should reuse the same OmniHarness thread."""
        from app.channels.manager import ChannelManager
...
class TestChannelService:
    ...
    def test_service_urls_fall_back_to_env(self, monkeypatch):
        ...
        monkeypatch.setenv("OMNI_HARNESS_CHANNELS_LANGGRAPH_URL", "http://gateway:8001/api")
        monkeypatch.setenv("OMNI_HARNESS_CHANNELS_GATEWAY_URL", "http://gateway:8001")
    ...
    def test_from_app_config_uses_explicit_config(self):
        ...
        with patch("omniharness.config.app_config.get_app_config", side_effect=AssertionError("should not read global config")):

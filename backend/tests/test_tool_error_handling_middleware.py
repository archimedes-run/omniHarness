...
from omniharness.agents.middlewares.tool_error_handling_middleware import (
    ToolErrorHandlingMiddleware,
    build_subagent_runtime_middlewares,
)
from omniharness.agents.middlewares.view_image_middleware import ViewImageMiddleware
from omniharness.config.app_config import AppConfig, CircuitBreakerConfig
from omniharness.config.guardrails_config import GuardrailsConfig
...
def _stub_runtime_middleware_imports(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "omniharness.agents.middlewares.llm_error_handling_middleware",
        _module(
            "omniharness.agents.middlewares.llm_error_handling_middleware",
    ...
    monkeypatch.setitem(
        ...
        _module("omniharness.agents.middlewares.thread_data_middleware", ThreadDataMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "omniharness.sandbox.middleware",
        _module("omniharness.sandbox.middleware", SandboxMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "omniharness.agents.middlewares.dangling_tool_call_middleware",
        _module("omniharness.agents.middlewares.dangling_tool_call_middleware", DanglingToolCallMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "omniharness.agents.middlewares.sandbox_audit_middleware",
        _module("omniharness.agents.middlewares.sandbox_audit_middleware", SandboxAuditMiddleware=FakeMiddleware),
    )

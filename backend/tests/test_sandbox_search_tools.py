...
from omniharness.sandbox.local.local_sandbox import LocalSandbox
from omniharness.sandbox.search import GrepMatch, find_glob_matches, find_grep_matches
from omniharness.sandbox.tools import glob_tool, grep_tool, ls_tool
...
def test_glob_tool_returns_virtual_paths_and_ignores_common_dirs(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("omniharness.sandbox.tools.ensure_sandbox_initialized", lambda runtime: LocalSandbox(id="local"))
...
def test_glob_tool_supports_skills_virtual_paths(tmp_path, monkeypatch) -> None:
    ...
    with (
        ...
        patch("omniharness.sandbox.tools._get_skills_host_path", return_value=str(skills_dir)),
    ):
        result = glob_tool.func(
            runtime=runtime,
...
def test_grep_tool_truncates_results(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("omniharness.sandbox.tools.get_app_config", lambda: SimpleNamespace(get_tool_config=lambda name: None))
...
def test_aio_sandbox_glob_include_dirs_filters_nested_ignored(monkeypatch) -> None:
    with patch("omniharness.community.aio_sandbox.aio_sandbox.AioSandboxClient"):
...
def test_glob_tool_honors_smaller_requested_max_results(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("omniharness.sandbox.tools.ensure_sandbox_initialized", lambda runtime: LocalSandbox(id="local"))
    monkeypatch.setattr(
        "omniharness.sandbox.tools.get_app_config",

...
from omniharness.config.acp_config import ACPAgentConfig
from omniharness.config.extensions_config import ExtensionsConfig, McpServerConfig, set_extensions_config
from omniharness.tools.builtins.invoke_acp_agent_tool import (
...
def test_get_work_dir_uses_base_dir_when_no_thread_id(monkeypatch, tmp_path):
    ...
    from omniharness.config import paths as paths_module
...
def test_get_work_dir_uses_per_thread_path_when_thread_id_given(monkeypatch, tmp_path):
    ...
    from omniharness.config import paths as paths_module
    from omniharness.runtime import user_context as uc_module
...
async def test_invoke_acp_agent_skips_invalid_mcp_servers(monkeypatch, tmp_path, caplog):
    ...
    monkeypatch.setattr(paths_module, "get_paths", lambda: paths_module.Paths(base_dir=tmp_path))
    monkeypatch.setattr(
        "omniharness.tools.builtins.invoke_acp_agent_tool._build_acp_mcp_servers",
...
def test_get_available_tools_includes_invoke_acp_agent_when_agents_configured(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.tools.tools.get_app_config", lambda: fake_config)
    monkeypatch.setattr(
        "omniharness.config.extensions_config.ExtensionsConfig.from_file",
        classmethod(lambda cls: ExtensionsConfig(mcp_servers={}, skills={})),
    )
...
def test_get_available_tools_uses_explicit_app_config_for_acp_agents(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.tools.tools.is_host_bash_allowed", lambda config=None: True)
    monkeypatch.setattr("omniharness.config.acp_config.get_acp_agents", fail_get_acp_agents)
    monkeypatch.setattr("omniharness.tools.builtins.invoke_acp_agent_tool.build_invoke_acp_agent_tool", fake_build_invoke_acp_agent_tool)

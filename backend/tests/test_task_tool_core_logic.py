...
task_tool_module = importlib.import_module("omniharness.tools.builtins.task_tool")
...
def test_task_tool_threads_runtime_app_config_to_subagent_dependencies(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.tools.get_available_tools", fake_get_available_tools)
...
def test_task_tool_emits_running_and_completed_events(monkeypatch):
    ...
    monkeypatch.setattr(task_tool_module, "get_stream_writer", lambda: events.append)
    monkeypatch.setattr(task_tool_module.asyncio, "sleep", _no_sleep)
    # task_tool lazily imports from omniharness.tools at call time, so patch that module-level function.
    monkeypatch.setattr("omniharness.tools.get_available_tools", get_available_tools)
...
def test_task_tool_inherits_parent_skill_allowlist_for_default_subagent(monkeypatch):
    ...
    monkeypatch.setattr(task_tool_module, "get_stream_writer", lambda: events.append)
    monkeypatch.setattr(task_tool_module.asyncio, "sleep", _no_sleep)
    monkeypatch.setattr("omniharness.tools.get_available_tools", MagicMock(return_value=[]))

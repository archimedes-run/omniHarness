...
from omniharness.config.paths import Paths, join_host_path
...
def test_join_host_path_preserves_windows_drive_letter_style():
    base = r"C:\Users\demo\omni-harness\backend\.omni-harness"

    joined = join_host_path(base, "threads", "thread-9", "user-data", "outputs")

    assert joined == r"C:\Users\demo\omni-harness\backend\.omni-harness\threads\thread-9\user-data\outputs"
...
def test_get_thread_mounts_preserves_windows_host_path_style(tmp_path, monkeypatch):
    ...
    aio_mod = importlib.import_module("omniharness.community.aio_sandbox.aio_sandbox_provider")
    monkeypatch.setenv("OMNI_HARNESS_HOST_BASE_DIR", r"C:\Users\demo\omni-harness\backend\.omni-harness")
    monkeypatch.setattr(aio_mod, "get_paths", lambda: Paths(base_dir=tmp_path))
    monkeypatch.setattr(aio_mod, "get_effective_user_id", lambda: None)
    ...
    assert container_paths["/mnt/user-data/workspace"] == r"C:\Users\demo\omni-harness\backend\.omni-harness\threads\thread-10\user-data\workspace"
    assert container_paths["/mnt/user-data/uploads"] == r"C:\Users\demo\omni-harness\backend\.omni-harness\threads\thread-10\user-data\uploads"
    assert container_paths["/mnt/user-data/outputs"] == r"C:\Users\demo\omni-harness\backend\.omni-harness\threads\thread-10\user-data\outputs"
    assert container_paths["/mnt/acp-workspace"] == r"C:\Users\demo\omni-harness\backend\.omni-harness\threads\thread-10\acp-workspace"

...
from omniharness.agents.memory.message_processing import detect_correction, detect_reinforcement, filter_messages_for_memory
from omniharness.agents.memory.updater import _strip_upload_mentions_from_memory
...
class TestFilterMessagesForMemory:
    ...
    def test_upload_with_real_question_preserves_question(self):
        ...
        msgs = [
            _human(combined),
            _ai("The file contains: Hello OmniHarness."),
        ]
        result = filter_messages_for_memory(msgs)
        ...
        assert result[1].content == "The file contains: Hello OmniHarness."
...
class TestStripUploadMentionsFromMemory:
    ...
    def test_uploading_a_test_file_removed(self):
        ...
        mem = self._make_memory("User conducted a hands-on test by uploading a test file titled 'test_omniharness_memory_bug.txt'. User is also learning Python.")
        result = _strip_upload_mentions_from_memory(mem)
        summary = result["user"]["topOfMind"]["summary"]
        assert "test_omniharness_memory_bug.txt" not in summary

...
_THREAD_DATA = {
    "workspace_path": "/tmp/omni-harness/threads/t1/user-data/workspace",
    "uploads_path": "/tmp/omni-harness/threads/t1/user-data/uploads",
    "outputs_path": "/tmp/omni-harness/threads/t1/user-data/outputs",
}
...
def test_replace_virtual_path_maps_virtual_root_and_subpaths() -> None:
    assert Path(replace_virtual_path("/mnt/user-data/workspace/a.txt", _THREAD_DATA)).as_posix() == "/tmp/omni-harness/threads/t1/user-data/workspace/a.txt"
    assert Path(replace_virtual_path("/mnt/user-data", _THREAD_DATA)).as_posix() == "/tmp/omni-harness/threads/t1/user-data"
...
def test_replace_virtual_path_preserves_trailing_slash() -> None:
    ...
    assert result == "/tmp/omni-harness/threads/t1/user-data/workspace/"
...
def test_replace_virtual_path_preserves_trailing_slash_windows_style() -> None:
    ...
    """
    win_thread_data = {
        "workspace_path": r"C:\omni-harness\threads\t1\user-data\workspace",
        "uploads_path": r"C:\omni-harness\threads\t1\user-data\uploads",
        "outputs_path": r"C:\omni-harness\threads\t1\user-data\outputs",
    }
...
def test_replace_virtual_path_preserves_windows_style_for_nested_subdir_trailing_slash() -> None:
    ...
    assert result == "C:\\omni-harness\\threads\\t1\\user-data\\workspace\\subdir\\"
    assert "/" not in result, f"Mixed separators in Windows path: {result!r}"
...
def test_replace_virtual_paths_in_command_preserves_trailing_slash() -> None:
    ...
    assert "/tmp/omni-harness/threads/t1/user-data/workspace/" in result, f"Trailing slash lost in: {result!r}"
...
def test_mask_local_paths_in_output_hides_host_paths() -> None:
    output = "Created: /tmp/omni-harness/threads/t1/user-data/workspace/result.txt"
    masked = mask_local_paths_in_output(output, _THREAD_DATA)

    assert "/tmp/omni-harness/threads/t1/user-data" not in masked
...
def test_mask_local_paths_in_output_hides_skills_host_paths() -> None:
    ...
    with (
        patch("omniharness.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("omniharness.sandbox.tools._get_skills_host_path", return_value="/home/user/omni-harness/skills"),
    ):
        output = "Reading: /home/user/omni-harness/skills/public/bootstrap/SKILL.md"
        masked = mask_local_paths_in_output(output, _THREAD_DATA)

        assert "/home/user/omni-harness/skills" not in masked
...
def test_validate_local_tool_path_prioritizes_user_data_before_custom_mounts() -> None:
    from omniharness.config.sandbox_config import VolumeMountConfig

    mounts = [
        VolumeMountConfig(host_path="/tmp/host-user-data", container_path=VIRTUAL_PATH_PREFIX, read_only=False),
    ]
    with patch("omniharness.sandbox.tools._get_custom_mounts", return_value=mounts):
...
def test_validate_local_tool_path_rejects_traversal_in_skills() -> None:
    ...
    with patch("omniharness.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"):
        with pytest.raises(PermissionError, match="path traversal"):
...
def test_validate_local_tool_path_rejects_none_thread_data() -> None:
    ...
    from omniharness.sandbox.exceptions import SandboxRuntimeError
...
def test_resolve_skills_path_resolves_correctly() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/omni-harness/skills/public/bootstrap/SKILL.md"
...
def test_resolve_skills_path_resolves_root() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/omni-harness/skills"
...
def test_resolve_skills_path_raises_when_not_configured() -> None:
    ...
    with (
        patch("omniharness.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("omniharness.sandbox.tools._get_skills_host_path", return_value=None),
    ):
...
def test_replace_virtual_paths_in_command_replaces_skills_paths() -> None:
    ...
    with (
        ...
        assert "/home/user/omni-harness/skills/public/bootstrap/SKILL.md" in result
...
def test_replace_virtual_paths_in_command_replaces_both() -> None:
    ...
    with (
        patch("omniharness.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("omniharness.sandbox.tools._get_skills_host_path", return_value="/home/user/skills"),
    ):
        ...
        assert "/tmp/omni-harness/threads/t1/user-data/workspace/out.txt" in result
...
def test_bash_tool_rejects_host_bash_when_local_sandbox_default(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "omniharness.sandbox.tools.ensure_sandbox_initialized",
        lambda runtime: SimpleNamespace(execute_command=lambda command: pytest.fail("host bash should not execute")),
    )
    monkeypatch.setattr("omniharness.sandbox.tools.is_host_bash_allowed", lambda: False)
...
def test_bash_tool_blocks_relative_traversal_before_host_execution(monkeypatch) -> None:
    ...
    monkeypatch.setattr("omniharness.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
    monkeypatch.setattr("omniharness.sandbox.tools.is_host_bash_allowed", lambda: True)
...
def test_validate_local_tool_path_skills_custom_container_path() -> None:
    ...
    with patch("omniharness.sandbox.tools._get_skills_container_path", return_value="/custom/skills"):
        # Should not raise
        validate_local_tool_path(
...
def test_resolve_acp_workspace_path_resolves_correctly(tmp_path: Path) -> None:
    ...
    acp_dir.mkdir()
    with patch("omniharness.sandbox.tools._get_acp_workspace_host_path", return_value=str(acp_dir)):
...
def test_resolve_acp_workspace_path_raises_when_not_available() -> None:
    ...
    with patch("omniharness.sandbox.tools._get_acp_workspace_host_path", return_value=None):
...
def test_replace_virtual_paths_in_command_replaces_acp_workspace() -> None:
    ...
    acp_host = "/home/user/.omni-harness/acp-workspace"
    with patch("omniharness.sandbox.tools._get_acp_workspace_host_path", return_value=acp_host):
        ...
        assert "/tmp/omni-harness/threads/t1/user-data/outputs/hello.py" in result
...
def test_apply_cwd_prefix_prepends_workspace() -> None:
    ...
    assert "/tmp/omni-harness/threads/t1/user-data/workspace" in result
...
def test_validate_local_bash_command_paths_allows_mcp_filesystem_paths() -> None:
    ...
    from omniharness.config.extensions_config import ExtensionsConfig, McpServerConfig
    ...
    with patch("omniharness.config.extensions_config.get_extensions_config", return_value=mock_config):
        ...
        with patch("omniharness.config.extensions_config.get_extensions_config", return_value=disabled_config):
            with pytest.raises(PermissionError, match="Unsafe absolute paths"):
                validate_local_bash_command_paths("ls /mnt/d/workspace", _THREAD_DATA)
...
def test_is_custom_mount_path_recognises_configured_mounts() -> None:
    with patch("omniharness.sandbox.tools._get_custom_mounts", return_value=_mock_custom_mounts()):
...
def test_get_custom_mounts_caching(monkeypatch, tmp_path) -> None:
    ...
    from omniharness.config.sandbox_config import SandboxConfig, VolumeMountConfig
    ...
    mock_sandbox = SandboxConfig(use="omniharness.sandbox.local:LocalSandboxProvider", mounts=mounts)
    mock_config = SimpleNamespace(sandbox=mock_sandbox)

    with patch("omniharness.config.get_app_config", return_value=mock_config):
...
def test_str_replace_parallel_updates_should_preserve_both_edits(monkeypatch) -> None:
    ...
    monkeypatch.setattr("omniharness.sandbox.tools.ensure_sandbox_initialized", lambda runtime: sandbox)
    monkeypatch.setattr("omniharness.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
    monkeypatch.setattr("omniharness.sandbox.tools.is_local_sandbox", lambda runtime: False)
...
def test_file_operation_lock_memory_cleanup() -> None:
    ...
    from omniharness.sandbox.file_operation_lock import _FILE_OPERATION_LOCKS, get_file_operation_lock

    class MockSandbox:
        id = "test_cleanup_sandbox"

    test_path = "/tmp/omni-harness/memory_leak_test_file.txt"

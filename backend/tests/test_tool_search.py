...
from omniharness.tools.builtins.tool_search import (
...
class TestToolSearchTool:
    def test_no_registry(self):
        from omniharness.tools.builtins.tool_search import tool_search
...
class TestDeferredToolsPromptSection:
    ...
    def _mock_app_config(self, monkeypatch):
        ...
        from omniharness.config.tool_search_config import ToolSearchConfig

        mock_config = MagicMock()
        mock_config.tool_search = ToolSearchConfig()  # disabled by default
        monkeypatch.setattr("omniharness.config.get_app_config", lambda: mock_config)

    def test_empty_when_disabled(self):
        from omniharness.agents.lead_agent.prompt import get_deferred_tools_prompt_section
...
class TestDeferredToolFilterMiddleware:
    ...
    def _ensure_middlewares_package(self):
        ...
        That file replaces omniharness.agents and omniharness.agents.middlewares with
        ...
        mock_keys = [
            ...
            "omniharness.agents.middlewares.deferred_tool_filter_middleware",
        ]

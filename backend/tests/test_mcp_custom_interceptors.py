...
from omniharness.mcp.tools import get_mcp_tools
...
def _make_patches(*, interceptor_paths=None):
    ...
    return {
        ...
        "from_file": patch(
            "omniharness.config.extensions_config.ExtensionsConfig.from_file",
        ...
        "build_servers": patch(
            "omniharness.mcp.tools.build_servers_config",
            return_value={"test-server": {}},
        ),
        "oauth_headers": patch(
            "omniharness.mcp.tools.get_initial_oauth_headers",
            new_callable=AsyncMock,
            return_value={},
        ),
        "oauth_interceptor": patch(
            "omniharness.mcp.tools.build_oauth_tool_interceptor",
            return_value=None,
        ),
    }
...
def test_custom_interceptor_loaded_and_appended():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("omniharness.mcp.tools.resolve_variable", return_value=fake_builder),
    ):
...
def test_multiple_custom_interceptors():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("omniharness.mcp.tools.resolve_variable", side_effect=lambda path: builders[path]),
    ):
        asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_builder_returning_none_is_skipped():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("omniharness.mcp.tools.resolve_variable", return_value=lambda: None),
    ):
        asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_resolve_error_logs_warning_and_continues():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("omniharness.mcp.tools.resolve_variable", side_effect=ImportError("no such module")),
        patch("omniharness.mcp.tools.logger.warning") as mock_warn,
    ):
...
def test_custom_interceptor_builder_exception_logs_warning_and_continues():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("omniharness.mcp.tools.resolve_variable", return_value=exploding_builder),
        patch("omniharness.mcp.tools.logger.warning") as mock_warn,
    ):
        tools = asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_coexists_with_oauth_interceptor():
    ...
    with (
        ...
        p["build_servers"],
        p["oauth_headers"],
        patch("omniharness.mcp.tools.build_oauth_tool_interceptor", return_value=oauth_fn),
        patch("omniharness.mcp.tools.resolve_variable", return_value=lambda: custom_fn),
    ):
        asyncio.run(get_mcp_tools())
...
def test_mcp_interceptors_single_string_is_normalized():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("omniharness.mcp.tools.resolve_variable", return_value=lambda: fake_interceptor),
    ):
        asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_non_callable_return_logs_warning():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("omniharness.mcp.tools.resolve_variable", return_value=lambda: "not_a_callable"),
        patch("omniharness.mcp.tools.logger.warning") as mock_warn,
    ):
        asyncio.run(get_mcp_tools())

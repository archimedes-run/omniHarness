...
import omniharness.config.app_config as app_config_module
from omniharness.config.acp_config import load_acp_config_from_dict
from omniharness.config.agents_api_config import get_agents_api_config, load_agents_api_config_from_dict
from omniharness.config.app_config import AppConfig, get_app_config, reset_app_config
from omniharness.config.checkpointer_config import get_checkpointer_config, load_checkpointer_config_from_dict
from omniharness.config.guardrails_config import get_guardrails_config, load_guardrails_config_from_dict
from omniharness.config.memory_config import get_memory_config, load_memory_config_from_dict
from omniharness.config.stream_bridge_config import get_stream_bridge_config, load_stream_bridge_config_from_dict
from omniharness.config.subagents_config import get_subagents_app_config, load_subagents_config_from_dict
from omniharness.config.summarization_config import get_summarization_config, load_summarization_config_from_dict
from omniharness.config.title_config import get_title_config, load_title_config_from_dict
from omniharness.config.tool_search_config import get_tool_search_config, load_tool_search_config_from_dict
from omniharness.runtime.checkpointer import get_checkpointer, reset_checkpointer
from omniharness.runtime.store import get_store, reset_store
...
def test_app_config_defaults_missing_database_to_sqlite(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNI_HARNESS_EXTENSIONS_CONFIG_PATH", str(extensions_path))

    config = AppConfig.from_file(str(config_path))

    assert config.database.backend == "sqlite"
    assert config.database.sqlite_dir == ".omni-harness/data"
...
def test_get_app_config_reloads_when_file_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNI_HARNESS_CONFIG_PATH", str(config_path))
    monkeypatch.setenv("OMNI_HARNESS_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    reset_app_config()
...
def test_get_app_config_reloads_when_config_path_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNI_HARNESS_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    monkeypatch.setenv("OMNI_HARNESS_CONFIG_PATH", str(config_a))
    reset_app_config()
    ...
    try:
        ...
        monkeypatch.setenv("OMNI_HARNESS_CONFIG_PATH", str(config_b))

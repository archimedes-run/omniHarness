...
from omniharness.config.checkpointer_config import (
    CheckpointerConfig,
...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("omniharness.runtime.checkpointer.provider.get_app_config", side_effect=FileNotFoundError):
            cp = get_checkpointer()
    ...
    def test_sqlite_creates_parent_dir(self):
        ...
        .omni-harness directory has not been created).
        """
        ...
        with (
            ...
            patch("omniharness.runtime.checkpointer.provider.ensure_sqlite_parent_dir") as mock_ensure,
            patch(
                "omniharness.runtime.checkpointer.provider.resolve_sqlite_conn_str",
    ...
    def test_sqlite_ensure_parent_dir_before_connect(self):
        ...
        with (
            patch.dict(sys.modules, {"langgraph.checkpoint.sqlite": mock_module}),
            patch(
                "omniharness.runtime.checkpointer.provider.ensure_sqlite_parent_dir",
...
class TestAsyncCheckpointer:
    ...
    async def test_sqlite_creates_parent_dir_via_to_thread(self):
        ...
        from omniharness.runtime.checkpointer.async_provider import make_checkpointer
        ...
        with (
            patch("omniharness.runtime.checkpointer.async_provider.get_app_config", return_value=mock_config),
            patch.dict(sys.modules, {"langgraph.checkpoint.sqlite.aio": mock_module}),
            patch("omniharness.runtime.checkpointer.async_provider.asyncio.to_thread", new_callable=AsyncMock) as mock_to_thread,
            patch(
                "omniharness.runtime.checkpointer.async_provider.resolve_sqlite_conn_str",
...
# ---------------------------------------------------------------------------
# OmniHarnessClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """OmniHarnessClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            patch("omniharness.client.get_app_config", return_value=config_mock),
            patch("omniharness.client.create_agent", side_effect=fake_create_agent),
            patch("omniharness.client.create_chat_model", return_value=MagicMock()),
            patch("omniharness.client._build_middlewares", return_value=[]),
            patch("omniharness.client.apply_prompt_template", return_value=""),
            patch("omniharness.client.OmniHarnessClient._get_tools", return_value=[]),
        ):
            client = OmniHarnessClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = OmniHarnessClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)

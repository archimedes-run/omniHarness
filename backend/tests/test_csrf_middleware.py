...
def test_auth_post_rejects_malformed_origin_with_path():
    ...
    response = client.post(
        "/api/v1/auth/login/local",
        headers={"Origin": "https://omniharness.example/path"},
    )
...
def test_auth_post_rejects_malformed_origin_with_invalid_port():
    ...
    response = client.post(
        "/api/v1/auth/login/local",
        headers={"Origin": "https://omniharness.example:bad"},
    )
...
def test_auth_post_allows_same_origin_default_port_equivalence():
    ...
    response = client.post(
        "/api/v1/auth/login/local",
        headers={"Origin": "https://omniharness.example:443"},
    )
...
def test_auth_post_allows_forwarded_same_origin():
    ...
    response = client.post(
        ...
        headers={
            ...
            "X-Forwarded-Host": "omniharness.example, internal:8000",
        },
    )
...
def test_auth_post_allows_rfc_forwarded_same_origin():
    ...
    response = client.post(
        ...
        headers={
            "Origin": "https://omniharness.example",
            "Forwarded": "proto=https;host=omniharness.example",
        },
    )

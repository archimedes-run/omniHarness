...
from omniharness.config.subagents_config import CustomSubagentConfig, SubagentsAppConfig
...
def test_apply_prompt_template_threads_explicit_app_config_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr("omniharness.config.memory_config.get_memory_config", fail_get_memory_config)
...
def test_apply_prompt_template_threads_explicit_app_config_to_subagents_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr("omniharness.config.subagents_config.get_subagents_app_config", fail_get_subagents_app_config)
    monkeypatch.setattr(prompt_module, "get_or_new_skill_storage", lambda app_config=None: SimpleNamespace(load_skills=lambda enabled_only=True: []))
    monkeypatch.setattr(prompt_module, "get_agent_soul", lambda agent_name=None: "")
...
def test_get_memory_context_uses_explicit_app_config_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.config.memory_config.get_memory_config", fail_get_memory_config)
    monkeypatch.setattr("omniharness.runtime.user_context.get_effective_user_id", lambda: "user-1")
    monkeypatch.setattr("omniharness.agents.memory.get_memory_data", fake_get_memory_data)
    monkeypatch.setattr("omniharness.agents.memory.format_memory_for_injection", fake_format_memory_for_injection)

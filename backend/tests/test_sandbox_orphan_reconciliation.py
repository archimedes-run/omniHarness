...
from omniharness.community.aio_sandbox.sandbox_info import SandboxInfo
...
def test_backend_list_running_default_returns_empty():
    ...
    from omniharness.community.aio_sandbox.backend import SandboxBackend
...
def _make_local_backend():
    ...
    from omniharness.community.aio_sandbox.local_backend import LocalContainerBackend
...
def test_list_running_returns_containers(monkeypatch):
    ...
    _mock_ps_and_inspect(
        monkeypatch,
        ps_output="omni-harness-sandbox-abc12345\nomni-harness-sandbox-def67890\n",
        inspect_payload=[
            _make_inspect_entry("omni-harness-sandbox-abc12345", "2026-04-08T01:22:50.000000000Z", "8081"),
            _make_inspect_entry("omni-harness-sandbox-def67890", "2026-04-08T02:22:50.000000000Z", "8082"),
        ],
    )
...
def test_list_running_skips_non_matching_names(monkeypatch):
    ...
    _mock_ps_and_inspect(
        ...
        inspect_payload=[
            _make_inspect_entry("omni-harness-sandbox-abc12345", "2026-04-08T01:22:50Z", "8081"),
        ],
    )
...
def test_list_running_includes_containers_without_port(monkeypatch):
    ...
    _mock_ps_and_inspect(
        ...
        ps_output="omni-harness-sandbox-abc12345\n",
        inspect_payload=[
            _make_inspect_entry("omni-harness-sandbox-abc12345", "2026-04-08T01:22:50Z", host_port=None),
        ],
    )
...
def test_list_running_handles_malformed_inspect_json(monkeypatch):
    ...
    def mock_run(cmd, **kwargs):
        ...
        if len(cmd) >= 2 and cmd[1] == "ps":
            result.returncode = 0
            result.stdout = "omni-harness-sandbox-abc12345\n"
            result.stderr = ""
        else:
            result.returncode = 0
...
def test_list_running_uses_single_batch_inspect_call(monkeypatch):
    ...
    def mock_run(cmd, **kwargs):
        ...
        if len(cmd) >= 2 and cmd[1] == "ps":
            result.returncode = 0
            result.stdout = "omni-harness-sandbox-a\nomni-harness-sandbox-b\nomni-harness-sandbox-c\n"
            result.stderr = ""
            return result
        if len(cmd) >= 2 and cmd[1] == "inspect":
            inspect_call_count["count"] += 1
            # Expect all three names passed in a single call
            assert cmd[2:] == ["omni-harness-sandbox-a", "omni-harness-sandbox-b", "omni-harness-sandbox-c"]
            result.returncode = 0
            result.stdout = json.dumps(
                [
                    _make_inspect_entry("omni-harness-sandbox-a", "2026-04-08T01:22:50Z", "8081"),
                    _make_inspect_entry("omni-harness-sandbox-b", "2026-04-08T01:22:50Z", "8082"),
                    _make_inspect_entry("omni-harness-sandbox-c", "2026-04-08T01:22:50Z", "8083"),
                ]
            )
            result.stderr = ""
            return result
...
def test_parse_docker_timestamp_with_nanoseconds():
    ...
    from omniharness.community.aio_sandbox.local_backend import _parse_docker_timestamp
...
def test_extract_host_port_returns_mapped_port():
    from omniharness.community.aio_sandbox.local_backend import _extract_host_port
...
def _make_provider_for_reconciliation():
    ...
    """
    aio_mod = importlib.import_module("omniharness.community.aio_sandbox.aio_sandbox_provider")
...
def test_reconcile_adopts_old_containers_into_warm_pool():
    ...
    old_info = SandboxInfo(
        ...
        container_name="omni-harness-sandbox-old12345",
...
def test_reconcile_adopts_young_containers():
    ...
    young_info = SandboxInfo(
        ...
        container_name="omni-harness-sandbox-young123",
...
def test_reconcile_mixed_containers_all_adopted():
    ...
    old_info = SandboxInfo(
        sandbox_id="old_one",
        sandbox_url="http://localhost:8081",
        container_name="omni-harness-sandbox-old_one",
        created_at=now - 1200,
    )
    young_info = SandboxInfo(
        sandbox_id="young_one",
        sandbox_url="http://localhost:8082",
        container_name="omni-harness-sandbox-young_one",
        created_at=now - 60,
    )
...
def test_reconcile_skips_already_tracked_containers():
    ...
    existing_info = SandboxInfo(
        ...
        sandbox_url="http://localhost:8081",
        container_name="omni-harness-sandbox-existing1",
        created_at=now - 1200,
    )

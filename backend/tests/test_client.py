...
from omniharness.uploads.manager import PathTraversalError
...
def client(mock_app_config, tmp_path):
    """Create a OmniHarnessClient with mocked config loading."""
    import omniharness.skills.storage as _storage_mod
    from omniharness.skills.storage.local_skill_storage import LocalSkillStorage

    _storage_mod._default_skill_storage = LocalSkillStorage(host_path=str(tmp_path))
    with patch("omniharness.client.get_app_config", return_value=mock_app_config):
        return OmniHarnessClient()


@pytest.fixture
def allow_skill_security_scan():
    async def _scan(*args, **kwargs):
        from omniharness.skills.security_scanner import ScanResult

        return ScanResult(decision="allow", reason="ok")

    with patch("omniharness.skills.installer.scan_skill_content", _scan):
        yield
...
class TestClientInit:
    ...
    def test_custom_params(self, mock_app_config):
        ...
        with patch("omniharness.client.get_app_config", return_value=mock_app_config):
            c = OmniHarnessClient(model_name="gpt-4", thinking_enabled=False, subagent_enabled=True, plan_mode=True, agent_name="test-agent", available_skills={"skill1", "skill2"}, middlewares=[mock_middleware])
    ...
    def test_invalid_agent_name(self, mock_app_config):
        with patch("omniharness.client.get_app_config", return_value=mock_app_config):
            with pytest.raises(ValueError, match="Invalid agent name"):
                OmniHarnessClient(agent_name="invalid name with spaces!")
            with pytest.raises(ValueError, match="Invalid agent name"):
                OmniHarnessClient(agent_name="../path/traversal")

    def test_custom_config_path(self, mock_app_config):
        with (
            patch("omniharness.client.reload_app_config") as mock_reload,
            patch("omniharness.client.get_app_config", return_value=mock_app_config),
        ):
            OmniHarnessClient(config_path="/tmp/custom.yaml")
            mock_reload.assert_called_once_with("/tmp/custom.yaml")

    def test_checkpointer_stored(self, mock_app_config):
        cp = MagicMock()
        with patch("omniharness.client.get_app_config", return_value=mock_app_config):
            c = OmniHarnessClient(checkpointer=cp)
...
class TestConfigQueries:
    ...
    def test_list_skills(self, client):
        ...
        with patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[skill]) as mock_load:
    ...
    def test_list_skills_enabled_only(self, client):
        with patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[]) as mock_load:
            client.list_skills(enabled_only=True)
            mock_load.assert_called_once_with(enabled_only=True)

    def test_get_memory(self, client):
        memory = {"version": "1.0", "facts": []}
        with patch("omniharness.agents.memory.updater.get_memory_data", return_value=memory) as mock_mem:
...
class TestStream:
    ...
    def test_custom_events_are_forwarded(self, client):
        ...
        # see OmniHarnessClient.stream() docstring and GitHub issue #1969.
    ...
    def test_messages_mode_emits_token_deltas(self, client):
        ...
        Regression for archimedes-run/omni-harness#1969 — before the fix the client
...
class TestExtractText:
    def test_string(self):
        assert OmniHarnessClient._extract_text("hello") == "hello"
    ...
    def test_list_text_blocks(self):
        ...
        assert OmniHarnessClient._extract_text(content) == "first\nsecond"

    def test_list_plain_strings(self):
        assert OmniHarnessClient._extract_text(["a", "b"]) == "a\nb"

    def test_empty_list(self):
        assert OmniHarnessClient._extract_text([]) == ""

    def test_other_type(self):
        assert OmniHarnessClient._extract_text(42) == "42"
...
class TestEnsureAgent:
    def test_creates_agent(self, client):
        ...
        with (
            patch("omniharness.client.create_chat_model"),
            patch("omniharness.client.create_agent", return_value=mock_agent),
            patch("omniharness.client._build_middlewares", return_value=[]) as mock_build_middlewares,
            patch("omniharness.client.apply_prompt_template", return_value="prompt") as mock_apply_prompt,
            patch.object(client, "_get_tools", return_value=[]),
            patch("omniharness.runtime.checkpointer.get_checkpointer", return_value=MagicMock()),
        ):
    ...
    def test_uses_default_checkpointer_when_available(self, client):
        ...
        with (
            patch("omniharness.client.create_chat_model"),
            patch("omniharness.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("omniharness.client._build_middlewares", return_value=[]),
            patch("omniharness.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("omniharness.runtime.checkpointer.get_checkpointer", return_value=mock_checkpointer),
        ):
            client._ensure_agent(config)
    ...
    def test_injects_custom_middlewares(self, client):
        ...
        with (
            patch("omniharness.client.create_chat_model"),
            patch("omniharness.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("omniharness.client._build_middlewares", side_effect=fake_build_middlewares),
            patch("omniharness.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("omniharness.runtime.checkpointer.get_checkpointer", return_value=MagicMock()),
        ):
    ...
    def test_skips_default_checkpointer_when_unconfigured(self, client):
        ...
        with (
            ...
            patch("omniharness.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("omniharness.runtime.checkpointer.get_checkpointer", return_value=None),
        ):
            client._ensure_agent(config)
...
class TestThreadQueries:
    ...
    def test_list_threads_fallback_checkpointer(self, client):
        ...
        with patch("omniharness.runtime.checkpointer.provider.get_checkpointer", return_value=mock_checkpointer):
...
class TestMcpConfig:
    def test_get_mcp_config(self, client):
        ...
        with patch("omniharness.client.get_extensions_config", return_value=ext_config):
    ...
    def test_update_mcp_config(self, client):
        ...
        try:
            ...
            with (
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("omniharness.client.get_extensions_config", return_value=current_config),
                patch("omniharness.client.reload_extensions_config", return_value=reloaded_config),
            ):
...
class TestSkillsManagement:
    ...
    def test_get_skill_found(self, client):
        ...
        with patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[skill]):
            result = client.get_skill("test-skill")
        assert result is not None
        assert result["name"] == "test-skill"

    def test_get_skill_not_found(self, client):
        with patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[]):
    ...
    def test_update_skill(self, client):
        ...
        try:
            ...
            with (
                patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[skill], [updated_skill]]),
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("omniharness.client.get_extensions_config", return_value=ext_config),
                patch("omniharness.client.reload_extensions_config"),
            ):
    ...
    def test_install_skill(self, client, allow_skill_security_scan):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("omniharness.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))):
...
class TestMemoryManagement:
    def test_import_memory(self, client):
        ...
        with patch("omniharness.agents.memory.updater.import_memory_data", return_value=imported) as mock_import:
    ...
    def test_reload_memory(self, client):
        ...
        with patch("omniharness.agents.memory.updater.reload_memory_data", return_value=data):
            result = client.reload_memory()
        assert result == data

    def test_clear_memory(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("omniharness.agents.memory.updater.clear_memory_data", return_value=data):
            result = client.clear_memory()
        assert result == data

    def test_create_memory_fact(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("omniharness.agents.memory.updater.create_memory_fact", return_value=data) as create_fact:
    ...
    def test_delete_memory_fact(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("omniharness.agents.memory.updater.delete_memory_fact", return_value=data) as delete_fact:
    ...
    def test_update_memory_fact(self, client):
        data = {"version": "1.0", "facts": []}
        with patch("omniharness.agents.memory.updater.update_memory_fact", return_value=data) as update_fact:
    ...
    def test_get_memory_config(self, client):
        config = MagicMock()
        config.enabled = True
        config.storage_path = ".omni-harness/memory.json"
        ...
        with patch("omniharness.config.memory_config.get_memory_config", return_value=config):
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("omniharness.config.memory_config.get_memory_config", return_value=config),
            patch("omniharness.agents.memory.updater.get_memory_data", return_value=data),
        ):
...
class TestUploads:
    def test_upload_files(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("omniharness.client.get_uploads_dir", return_value=uploads_dir), patch("omniharness.client.ensure_uploads_dir", return_value=uploads_dir):
    ...
    def test_upload_files_reuses_single_executor_inside_event_loop(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.client.get_uploads_dir", return_value=uploads_dir),
                patch("omniharness.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("omniharness.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("omniharness.utils.file_conversion.convert_file_to_markdown", side_effect=fake_convert),
    ...
    def test_delete_upload_not_found(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            with patch("omniharness.client.get_uploads_dir", return_value=Path(tmp)):
                with pytest.raises(FileNotFoundError):
...
class TestArtifacts:
    def test_get_artifact(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("omniharness.client.get_paths", return_value=paths):
...
class TestScenarioConfigManagement:
    ...
    def test_mcp_update_then_skill_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("omniharness.client.get_extensions_config", return_value=current_config),
                patch("omniharness.client.reload_extensions_config", return_value=reloaded_config),
            ):
            ...
            with (
                patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[skill], [toggled]]),
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("omniharness.client.get_extensions_config", return_value=ext_config),
                patch("omniharness.client.reload_extensions_config"),
            ):
...
class TestScenarioAgentRecreation:
    ...
    def test_different_model_triggers_rebuild(self, client):
        ...
        with (
            patch("omniharness.client.create_chat_model"),
            patch("omniharness.client.create_agent", side_effect=fake_create_agent),
            patch("omniharness.client._build_middlewares", return_value=[]),
            patch("omniharness.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("omniharness.runtime.checkpointer.get_checkpointer", return_value=MagicMock()),
...
class TestScenarioThreadIsolation:
    ...
    def test_uploads_isolated_per_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("omniharness.client.get_uploads_dir", side_effect=get_dir), patch("omniharness.client.ensure_uploads_dir", side_effect=get_dir):
...
class TestScenarioMemoryWorkflow:
    ...
    def test_memory_full_lifecycle(self, client):
        ...
        with patch("omniharness.agents.memory.updater.get_memory_data", return_value=initial_data):
            mem = client.get_memory()
        assert len(mem["facts"]) == 1

        with patch("omniharness.agents.memory.updater.reload_memory_data", return_value=updated_data):
            refreshed = client.reload_memory()
        assert len(refreshed["facts"]) == 2

        with (
            patch("omniharness.config.memory_config.get_memory_config", return_value=config),
            patch("omniharness.agents.memory.updater.get_memory_data", return_value=updated_data),
        ):
...
class TestScenarioSkillInstallAndUse:
    ...
    def test_install_then_toggle(self, client, allow_skill_security_scan):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[installed_skill]):
                skills_result = client.list_skills()
            ...
            with (
                patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[installed_skill], [disabled_skill]]),
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("omniharness.client.get_extensions_config", return_value=ext_config),
                patch("omniharness.client.reload_extensions_config"),
            ):
    ...
    def test_upload_convertible_file_conversion_failure(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("omniharness.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("omniharness.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("omniharness.utils.file_conversion.convert_file_to_markdown", side_effect=Exception("conversion failed")),
            ):
...
class TestGatewayConformance:
    """Validate that OmniHarnessClient return dicts conform to Gateway Pydantic response models.
    ...
    def test_install_skill(self, client, tmp_path, allow_skill_security_scan):
        ...
        with patch("omniharness.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(tmp_path))):
            result = client.install_skill(archive)
    ...
    def test_update_mcp_config(self, client, tmp_path):
        ...
        with (
            patch("omniharness.client.get_extensions_config", return_value=ext_config),
            patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
            patch("omniharness.client.reload_extensions_config", return_value=ext_config),
        ):
    ...
    def test_get_memory_config(self, client):
        ...
        mem_cfg.storage_path = ".omni-harness/memory.json"
        ...
        with patch("omniharness.config.memory_config.get_memory_config", return_value=mem_cfg):
            result = client.get_memory_config()
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("omniharness.config.memory_config.get_memory_config", return_value=mem_cfg),
            patch("omniharness.agents.memory.updater.get_memory_data", return_value=memory_data),
        ):
            result = client.get_memory_status()
...
class TestInstallSkillSecurity:
    ...
    def test_zip_bomb_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            from omniharness.skills import installer as _installer
            ...
            with (
                patch("omniharness.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("omniharness.skills.installer.safe_extract_skill_archive", side_effect=patched_extract),
            ):
    ...
    def test_invalid_skill_name_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("omniharness.skills.validation._validate_skill_frontmatter", return_value=(True, "OK", "../evil")),
            ):
    ...
    def test_existing_skill_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("omniharness.skills.validation._validate_skill_frontmatter", return_value=(True, "OK", "dupe-skill")),
            ):
    ...
    def test_invalid_frontmatter_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("omniharness.skills.validation._validate_skill_frontmatter", return_value=(False, "Missing name field", "")),
            ):
...
class TestAtomicWriteJson:
    def test_temp_file_cleaned_on_serialization_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with pytest.raises(TypeError):
                OmniHarnessClient._atomic_write_json(target, bad_data)
    ...
    def test_happy_path_writes_atomically(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            OmniHarnessClient._atomic_write_json(target, data)
...
class TestConfigUpdateErrors:
    def test_update_mcp_config_no_config_file(self, client):
        ...
        with patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=None):
    ...
    def test_update_skill_no_config_file(self, client):
        ...
        with (
            patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[skill]),
            patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=None),
        ):
            with pytest.raises(FileNotFoundError, match="Cannot locate"):
    ...
    def test_update_skill_disappears_after_write(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[skill], []]),
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("omniharness.client.get_extensions_config", return_value=ext_config),
                patch("omniharness.client.reload_extensions_config"),
            ):
...
class TestSerializeMessage:
    def test_system_message(self):
        ...
        result = OmniHarnessClient._serialize_message(msg)
...
class TestBugListUploadsDeadCode:
    ...
    def test_list_uploads_on_fresh_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("omniharness.uploads.manager.get_paths", return_value=mock_paths):
...
class TestBugAgentInvalidationInconsistency:
    ...
    def test_update_mcp_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("omniharness.client.get_extensions_config", return_value=current_config),
                patch("omniharness.client.reload_extensions_config", return_value=reloaded),
            ):
    ...
    def test_update_skill_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("omniharness.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[skill], [updated]]),
                patch("omniharness.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("omniharness.client.get_extensions_config", return_value=ext_config),
                patch("omniharness.client.reload_extensions_config"),
            ):

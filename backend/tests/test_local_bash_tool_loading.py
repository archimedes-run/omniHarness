...
def _make_config(*, allow_host_bash: bool, sandbox_use: str = "omniharness.sandbox.local:LocalSandboxProvider", extra_tools: list[SimpleNamespace] | None = None):
    return SimpleNamespace(
        tools=[
            SimpleNamespace(name="bash", group="bash", use="omniharness.sandbox.tools:bash_tool"),
...
def test_get_available_tools_hides_bash_for_default_local_sandbox(monkeypatch):
    monkeypatch.setattr("omniharness.tools.tools.get_app_config", lambda: _make_config(allow_host_bash=False))
    monkeypatch.setattr(
        "omniharness.tools.tools.resolve_variable",
...
def test_get_available_tools_keeps_bash_when_explicitly_enabled(monkeypatch):
    monkeypatch.setattr("omniharness.tools.tools.get_app_config", lambda: _make_config(allow_host_bash=True))
    monkeypatch.setattr(
        "omniharness.tools.tools.resolve_variable",
        lambda use, _: SimpleNamespace(name="bash" if "bash" in use else "ls"),
    )
...
def test_get_available_tools_hides_renamed_host_bash_alias(monkeypatch):
    config = _make_config(
        ...
        extra_tools=[SimpleNamespace(name="shell", group="bash", use="omniharness.sandbox.tools:bash_tool")],
    )
    monkeypatch.setattr("omniharness.tools.tools.get_app_config", lambda: config)
    monkeypatch.setattr(
        "omniharness.tools.tools.resolve_variable",

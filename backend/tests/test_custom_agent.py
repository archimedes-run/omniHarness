...
from omniharness.config.agents_api_config import AgentsApiConfig, get_agents_api_config, set_agents_api_config
...
class TestLoadAgentConfig:
    def test_load_valid_config(self, tmp_path):
        ...
        with patch("omniharness.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from omniharness.config.agents_config import load_agent_config
...
class TestLoadAgentSoul:
    def test_reads_soul_file(self, tmp_path):
        ...
        with patch("omniharness.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from omniharness.config.agents_config import AgentConfig, load_agent_soul
...
class TestListCustomAgents:
    def test_empty_when_no_agents_dir(self, tmp_path):
        with patch("omniharness.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from omniharness.config.agents_config import list_custom_agents
...
class TestMemoryFilePath:
    def test_global_memory_path(self, tmp_path):
        ...
        from omniharness.agents.memory.storage import FileMemoryStorage
        from omniharness.config.memory_config import MemoryConfig

        with (
            patch("omniharness.agents.memory.storage.get_paths", return_value=_make_paths(tmp_path)),
            patch("omniharness.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_path="")),
        ):
...
def agent_client(tmp_path):
    ...
    with patch("omniharness.config.agents_config.get_paths", return_value=paths_instance), patch.object(agents_router, "get_paths", return_value=paths_instance):

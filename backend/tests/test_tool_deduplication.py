...
@patch("omniharness.tools.tools.get_app_config")
@patch("omniharness.tools.tools.is_host_bash_allowed", return_value=True)
@patch("omniharness.tools.tools.reset_deferred_registry")
def test_no_duplicates_returned(mock_reset, mock_bash, mock_cfg):
    ...
    with patch("omniharness.tools.tools.BUILTIN_TOOLS", [_tool_alpha, _tool_alpha_dup, _tool_beta]):
...
def test_first_occurrence_wins(mock_reset, mock_bash, mock_cfg):
    ...
    with patch("omniharness.tools.tools.BUILTIN_TOOLS", [sentinel_alpha, sentinel_alpha_dup, _tool_beta]):
        result = get_available_tools(include_mcp=False)
...
def test_duplicate_triggers_warning(mock_reset, mock_bash, mock_cfg, caplog):
    ...
    with patch("omniharness.tools.tools.BUILTIN_TOOLS", [_tool_alpha, _tool_alpha_dup]):
        with caplog.at_level(logging.WARNING, logger="omniharness.tools.tools"):

...
async def test_scan_skill_content_passes_run_name_to_model(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.skills.security_scanner.create_chat_model", lambda **kwargs: model)
...
async def test_scan_skill_content_blocks_when_model_unavailable(monkeypatch):
    config = SimpleNamespace(skill_evolution=SimpleNamespace(moderation_model_name=None))
    monkeypatch.setattr("omniharness.skills.security_scanner.get_app_config", lambda: config)
    monkeypatch.setattr("omniharness.skills.security_scanner.create_chat_model", lambda **kwargs: (_ for _ in ()).throw(RuntimeError("boom")))

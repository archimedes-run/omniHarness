...
omniharness.subagents.executor. This test file uses delayed import via fixture to test
...
_MOCKED_MODULE_NAMES = [
    "omniharness.agents",
    "omniharness.agents.thread_state",
    "omniharness.agents.middlewares",
    "omniharness.agents.middlewares.thread_data_middleware",
    "omniharness.sandbox",
    "omniharness.sandbox.middleware",
    "omniharness.sandbox.security",
    "omniharness.models",
    "omniharness.skills.storage",
]
...
def _setup_executor_classes():
    ...
    original_executor = sys.modules.get("omniharness.subagents.executor")

    # Remove mocked executor if exists (from conftest.py)
    if "omniharness.subagents.executor" in sys.modules:
        del sys.modules["omniharness.subagents.executor"]

    # Set up mocks
    for name in _MOCKED_MODULE_NAMES:
        sys.modules[name] = MagicMock()
    storage_module = ModuleType("omniharness.skills.storage")
    storage_module.get_or_new_skill_storage = lambda **kwargs: SimpleNamespace(load_skills=lambda *, enabled_only: [])
    sys.modules["omniharness.skills.storage"] = storage_module
    ...
    if original_executor is not None:
        sys.modules["omniharness.subagents.executor"] = original_executor
    elif "omniharness.subagents.executor" in sys.modules:
        del sys.modules["omniharness.subagents.executor"]
...
class TestAgentConstruction:
    ...
    def test_create_agent_threads_explicit_app_config_to_model_and_middlewares(
        ...
        import omniharness.config as config_module
        from omniharness.subagents import executor as executor_module
        ...
        monkeypatch.setitem(
            sys.modules,
            "omniharness.agents.middlewares.tool_error_handling_middleware",
            _module(
                "omniharness.agents.middlewares.tool_error_handling_middleware",
    ...
    async def test_load_skill_messages_uses_explicit_app_config_for_skill_storage(
        ...
        monkeypatch.setattr(sys.modules["omniharness.skills.storage"], "get_or_new_skill_storage", fake_get_or_new_skill_storage)
...
class TestCleanupBackgroundTask:
    ...
    def executor_module(self, _setup_executor_classes):
        ...
        from omniharness.subagents import executor

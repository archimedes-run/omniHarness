...
from omniharness.config import app_config as app_config_module
from omniharness.config import extensions_config as extensions_config_module
from omniharness.config import skills_config as skills_config_module
...
from omniharness.config.runtime_paths import project_root
...
def _clear_path_env(monkeypatch):
    for name in (
        ...
        "OMNI_HARNESS_EXTENSIONS_CONFIG_PATH",
        "OMNI_HARNESS_HOME",
        "OMNI_HARNESS_PROJECT_ROOT",
        "OMNI_HARNESS_SKILLS_PATH",
    ):
...
def test_default_runtime_paths_resolve_from_current_project(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / "config.yaml").write_text(
        yaml.safe_dump({"sandbox": {"use": "omniharness.sandbox.local:LocalSandboxProvider"}}),
        encoding="utf-8",
    )
    ...
    assert Paths().base_dir == tmp_path / ".omni-harness"
...
def test_deer_flow_project_root_overrides_current_directory(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("OMNI_HARNESS_PROJECT_ROOT", str(project_root))
    ...
    assert Paths().base_dir == project_root / ".omni-harness"
...
def test_deer_flow_skills_path_overrides_project_default(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("OMNI_HARNESS_SKILLS_PATH", "team-skills")
...
def test_deer_flow_project_root_must_exist(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("OMNI_HARNESS_PROJECT_ROOT", str(missing_root))
...
def test_deer_flow_project_root_must_be_directory(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("OMNI_HARNESS_PROJECT_ROOT", str(project_root_file))
...
def test_app_config_falls_back_to_legacy_when_project_root_lacks_config(tmp_path: Path, monkeypatch):
    """When OMNI_HARNESS_PROJECT_ROOT is unset and cwd has no config.yaml, the
...
def test_skills_config_falls_back_to_legacy_when_project_root_lacks_skills(tmp_path: Path, monkeypatch):
    """When OMNI_HARNESS_PROJECT_ROOT is unset and cwd has no `skills/`, the legacy
...
def test_extensions_config_falls_back_to_legacy_when_project_root_lacks_file(tmp_path: Path, monkeypatch):
    ...
    fake_paths_module_file = fake_backend / "packages" / "harness" / "omniharness" / "config" / "extensions_config.py"

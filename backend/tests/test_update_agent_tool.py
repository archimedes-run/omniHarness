...
from omniharness.tools.builtins.update_agent_tool import update_agent
...
def patched_paths(tmp_path: Path):
    ...
    with patch("omniharness.tools.builtins.update_agent_tool.get_paths", return_value=paths_mock):
        # load_agent_config also calls get_paths(); patch the same target it uses.
        with patch("omniharness.config.agents_config.get_paths", return_value=paths_mock):
            yield paths_mock
...
def stub_app_config():
    ...
    with patch("omniharness.tools.builtins.update_agent_tool.get_app_config", return_value=fake):
        yield fake
...
def test_update_agent_soul_failure_does_not_replace_config(tmp_path, patched_paths):
    ...
    with patch("omniharness.tools.builtins.update_agent_tool.tempfile.NamedTemporaryFile", side_effect=_explode_on_soul):
...
def test_update_agent_only_writes_under_current_user(tmp_path, patched_paths):
    ...
    from omniharness.runtime.user_context import reset_current_user, set_current_user
...
def test_update_agent_round_trips_known_fields(tmp_path, patched_paths):
    ...
    with patch("omniharness.tools.builtins.update_agent_tool.load_agent_config", return_value=fake_cfg):
        with patch("omniharness.tools.builtins.update_agent_tool.get_app_config", return_value=fake_app_config):
            update_agent.func(runtime=_runtime(), description="bumped")

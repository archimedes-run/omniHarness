...
from omniharness.sandbox.local.local_sandbox import LocalSandbox, PathMapping
from omniharness.sandbox.local.local_sandbox_provider import LocalSandboxProvider
...
class TestMultipleMounts:
    ...
    def test_execute_command_path_replacement(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("omniharness.sandbox.local.local_sandbox.subprocess.run", mock_run)
        monkeypatch.setattr("omniharness.sandbox.local.local_sandbox.LocalSandbox._get_shell", lambda self: "/bin/sh")
...
class TestLocalSandboxProviderMounts:
    def test_setup_path_mappings_uses_configured_skills_container_path_as_reserved_prefix(self, tmp_path):
        ...
        config = SimpleNamespace(
            skills=SimpleNamespace(container_path="/custom-skills", get_skills_path=lambda: skills_dir, use="omniharness.skills.storage.local_skill_storage:LocalSkillStorage"),
            sandbox=sandbox_config,
        )

        with patch("omniharness.config.get_app_config", return_value=config):
    ...
    def test_setup_path_mappings_skips_relative_host_path(self, tmp_path):
        ...
        config = SimpleNamespace(
            skills=SimpleNamespace(container_path="/mnt/skills", get_skills_path=lambda: skills_dir, use="omniharness.skills.storage.local_skill_storage:LocalSkillStorage"),
            sandbox=sandbox_config,
        )

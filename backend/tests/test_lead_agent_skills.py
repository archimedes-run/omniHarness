...
from omniharness.agents.lead_agent.prompt import get_skills_prompt_section
from omniharness.config.agents_config import AgentConfig
...
def test_get_skills_prompt_section_returns_empty_when_no_skills_match(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.agents.lead_agent.prompt._get_enabled_skills", lambda: skills)
...
def test_get_skills_prompt_section_includes_self_evolution_rules(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.agents.lead_agent.prompt._get_enabled_skills", lambda: skills)
    monkeypatch.setattr(
        "omniharness.config.get_app_config",
...
def test_get_skills_prompt_section_includes_self_evolution_rules_without_skills(monkeypatch):
    monkeypatch.setattr("omniharness.agents.lead_agent.prompt._get_enabled_skills", lambda: [])
    monkeypatch.setattr(
        "omniharness.config.get_app_config",
        lambda: SimpleNamespace(
            skills=SimpleNamespace(container_path="/mnt/skills"),
...
def test_get_skills_prompt_section_cache_respects_skill_evolution_toggle(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.config.get_app_config", lambda: config)
...
def test_get_skills_prompt_section_uses_explicit_config_for_enabled_skills(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.agents.lead_agent.prompt._get_enabled_skills", lambda: [_make_skill("global-skill")])
    monkeypatch.setattr("omniharness.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr(
        "omniharness.agents.lead_agent.prompt.get_or_new_skill_storage",
...
def test_make_lead_agent_empty_skills_passed_correctly(monkeypatch):
    ...
    from omniharness.agents.lead_agent import agent as lead_agent_module

    # Mock dependencies
    monkeypatch.setattr(lead_agent_module, "get_app_config", lambda: MagicMock())
    monkeypatch.setattr(lead_agent_module, "_resolve_model_name", lambda x=None, **kwargs: "default-model")
    monkeypatch.setattr(lead_agent_module, "create_chat_model", lambda **kwargs: "model")
    monkeypatch.setattr("omniharness.tools.get_available_tools", lambda **kwargs: [])
...
def test_make_lead_agent_filters_tools_from_available_skills(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.tools.get_available_tools", lambda **kwargs: [NamedTool("bash"), NamedTool("read_file"), NamedTool("web_search")])
...
def test_make_lead_agent_all_legacy_skills_preserve_all_tools(monkeypatch):
    ...
    monkeypatch.setattr("omniharness.tools.get_available_tools", lambda **kwargs: [NamedTool("bash"), NamedTool("read_file")])
...
def test_make_lead_agent_enforces_allowed_tools_when_skill_cache_is_cold(monkeypatch):
    ...
    from omniharness.agents.lead_agent import agent as lead_agent_module
    from omniharness.agents.lead_agent import prompt as prompt_module

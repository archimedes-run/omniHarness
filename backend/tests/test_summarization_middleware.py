...
from omniharness.agents.middlewares.summarization_middleware import OmniHarnessSummarizationMiddleware, SummarizationEvent
...
) -> OmniHarnessSummarizationMiddleware:
    model = MagicMock()
...
def test_memory_flush_hook_skips_when_memory_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    queue = MagicMock()
    monkeypatch.setattr("omniharness.agents.memory.summarization_hook.get_memory_config", lambda: MemoryConfig(enabled=False))
    monkeypatch.setattr("omniharness.agents.memory.summarization_hook.get_memory_queue", lambda: queue)
...
def test_memory_flush_hook_skips_when_thread_id_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    queue = MagicMock()
    monkeypatch.setattr("omniharness.agents.memory.summarization_hook.get_memory_config", lambda: MemoryConfig(enabled=True))
    monkeypatch.setattr("omniharness.agents.memory.summarization_hook.get_memory_queue", lambda: queue)

# Copilot Onboarding Instructions for OmniHarness
...
OmniHarness is a full-stack "super agent harness".
...
- `backend/packages/harness/omniharness/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/omniharness/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/omniharness/subagents/` - subagent registry/execution
- `backend/packages/harness/omniharness/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`omniharness.agents:make_lead_agent`)

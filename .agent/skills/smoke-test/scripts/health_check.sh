...
detect_mode() {
    ...
    if docker_available && docker ps --format "{{.Names}}" | grep -q "omni-harness"; then
        echo "docker"
    else
        echo "local"
    fi
...
if [ "$mode" = "docker" ]; then
    ...
    if docker ps --format "{{.Names}}" | grep -q "omni-harness"; then
        echo "✓ Containers are running:"
        docker ps --format "  - {{.Names}} ({{.Status}})"
    else
        echo "✗ No OmniHarness-related containers are running"
        all_passed=false
    fi
else

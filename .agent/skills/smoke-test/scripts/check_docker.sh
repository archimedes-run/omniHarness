...
if [ -n "$port_2026_usage" ]; then
    ...
    while IFS= read -r pid; do
        ...
        case "$process_command" in
            *[Dd]eer[Ff]low*|*[Dd]eerflow*|*[Nn]ginx*omniharness*|*omniharness/*[Nn]ginx*)
                omniharness_process_found=1
                ;;
        esac
    done <<EOF
...
EOF
    ...
    if [ "$omniharness_process_found" -eq 1 ]; then
        echo "✓ Port 2026 is occupied by OmniHarness"
    else
        echo "✗ Port 2026 must be free before starting OmniHarness"
        exit 1
    fi
else

...
This document lists common issues encountered during OmniHarness smoke testing and how to resolve them.
...
3. Or stop OmniHarness services first:
   ```bash
   make stop
   ```
...
2. Check nginx logs:
   ```bash
   cd docker && docker compose -p omni-harness-dev -f docker-compose-dev.yaml logs nginx
   ```
...
```bash
docker exec -it omni-harness-gateway sh
```

#### Clean Up All OmniHarness-Related Containers and Images
```bash
make docker-stop
cd docker && docker compose -p omni-harness-dev -f docker-compose-dev.yaml down -v
```
...
1. Check the GitHub issues for the project: https://github.com/archimedes-run/omni-harness/issues

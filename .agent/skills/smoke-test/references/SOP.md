# OmniHarness Smoke Test Standard Operating Procedure (SOP)

This document describes the detailed operating steps for each phase of the OmniHarness smoke test.

## Phase 1: Code Update Check

### 1.1 Confirm Current Directory

**Objective**: Verify that the current working directory is the OmniHarness project root.
...
**Success Criteria**: All ports are free, or they are occupied only by OmniHarness-related processes.
...
**Success Criteria**: Port 2026 is free, or it is occupied only by a OmniHarness-related process.
...
2. Confirm that the following containers are running:
   - `omni-harness-nginx`
   - `omni-harness-frontend`
   - `omni-harness-gateway`
   - `omni-harness-langgraph` (if not in gateway mode)

...
**Goal:** Add optional Langfuse observability support to OmniHarness while preserving existing LangSmith tracing and allowing both providers to be enabled at the same time.
...
Run: `cd backend && uv run ruff check packages/harness/omniharness/config/tracing_config.py packages/harness/omniharness/models/factory.py packages/harness/omniharness/tracing`

...
Full scan of `backend/.omni-harness/data/omniharness.db` `run_events`:
...
- `run_events` seq=1 → original human `"写一份关于omni-harness的详细技术报告"` ✓ (event store is fine)
...
**New wrapper middleware** in `backend/packages/harness/omniharness/agents/lead_agent/agent.py`:
...
1. **Synchronous path**. The upstream class has both `before_model` and `abefore_model`. Our wrapper only overrides the async variant. If any omni-harness code path ever uses the sync flow, those summarizations won't be captured. Mitigation: also override `before_model` and use `dispatch_custom_event` (sync variant) with the same pattern.

# OmniHarness Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the OmniHarness backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `omni-harness` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `omni-harness` | Kubernetes namespace for sandbox resources |
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `omni-harness` namespace
     - Create/read/delete Services in the `omni-harness` namespace
     - Read Namespaces (to create `omni-harness` if missing)
...
docker compose -p omni-harness-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec omni-harness-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec omni-harness-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec omni-harness-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n omni-harness -l sandbox-id=test-001

# Delete sandbox
docker exec omni-harness-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec omni-harness-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec omni-harness-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec omni-harness-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.omni-harness/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n omni-harness`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n omni-harness`
...
3. **Network Isolation**: Sandbox Pods run in the `omni-harness` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.

#!/usr/bin/env bash
# status.sh — Check OmniHarness status and list available resources.
#
# Usage:
...
case "$CMD" in
  health)
    echo "Checking OmniHarness at ${GATEWAY_URL}..."
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${GATEWAY_URL}/health" 2>/dev/null || echo "000")
    if [ "$HTTP_CODE" = "000" ]; then
      echo "UNREACHABLE — OmniHarness is not running at ${GATEWAY_URL}"
      exit 1
    elif [ "$HTTP_CODE" -ge 400 ]; then
      echo "ERROR — Health check returned HTTP ${HTTP_CODE}"
      exit 1
    else
      echo "OK — OmniHarness is running (HTTP ${HTTP_CODE})"
    fi
    ;;
  models)

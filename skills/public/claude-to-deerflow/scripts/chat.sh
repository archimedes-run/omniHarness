#!/usr/bin/env bash
# chat.sh — Send a message to OmniHarness and collect the streaming response.
#
# Usage:
...
#   OmniHarness_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   OmniHarness_URL          — Unified proxy base URL (default: http://localhost:2026)
#   OmniHarness_GATEWAY_URL  — Gateway API base URL (default: $OmniHarness_URL)
#   OmniHarness_LANGGRAPH_URL — LangGraph API base URL (default: $OmniHarness_URL/api/langgraph)
#
...
GATEWAY_URL="${OmniHarness_GATEWAY_URL:-$OmniHarness_URL}"
LANGGRAPH_URL="${OmniHarness_LANGGRAPH_URL:-$OmniHarness_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: OmniHarness is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure OmniHarness is running. Start it with: cd <OmniHarness-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from omniharness: {data_str}", file=sys.stderr)
            sys.exit(1)

---
name: claude-to-OmniHarness
description: "Interact with OmniHarness AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to OmniHarness for research/analysis, start a OmniHarness conversation thread, check OmniHarness status or health, list available models/skills/agents in OmniHarness, manage OmniHarness memory, upload files to OmniHarness threads, or delegate complex research tasks to OmniHarness. Also use when the user mentions OmniHarness, Omni Harness, or wants to run a deep research task that OmniHarness can handle."
---

# OmniHarness Skill

Communicate with a running OmniHarness instance via its HTTP API. OmniHarness is an AI agent platform
...
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$OmniHarness_GATEWAY_URL`          | REST endpoints (models, skills, memory, uploads) |
| LangGraph API  | 2024        | `$OmniHarness_LANGGRAPH_URL`        | Agent threads, runs, streaming   |
...
| `OmniHarness_GATEWAY_URL`  | `${OmniHarness_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `OmniHarness_LANGGRAPH_URL`| `${OmniHarness_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
OmniHarness_URL="${OmniHarness_URL:-http://localhost:2026}"
OmniHarness_GATEWAY_URL="${OmniHarness_GATEWAY_URL:-$OmniHarness_URL}"
OmniHarness_LANGGRAPH_URL="${OmniHarness_LANGGRAPH_URL:-$OmniHarness_URL/api/langgraph}"
```
...
```bash
curl -s "$OmniHarness_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$OmniHarness_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$OmniHarness_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$OmniHarness_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$OmniHarness_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$OmniHarness_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$OmniHarness_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$OmniHarness_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$OmniHarness_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$OmniHarness_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$OmniHarness_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$OmniHarness_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-OmniHarness/scripts/chat.sh "Your question here"
```
...
- If health check fails, OmniHarness is not running. Inform the user they need to start it.

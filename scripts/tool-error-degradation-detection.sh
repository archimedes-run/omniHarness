...
logging.getLogger("OmniHarness.agents.middlewares.tool_error_handling_middleware").setLevel(logging.CRITICAL)
...
def _build_sub_middlewares():
    try:
        from omniharness.agents.middlewares.tool_error_handling_middleware import build_subagent_runtime_middlewares
    except Exception:
        return [

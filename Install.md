...
This file is for coding agents. If the OmniHarness repository is not already cloned and open, clone `https://github.com/archimedes/omni-harness.git` first, then continue from the repository root.

## Goal

Bootstrap a OmniHarness local development workspace on the user's machine with the least risky path available.
...
- The OmniHarness repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch OmniHarness.
...
- If the current directory is not the OmniHarness repository root, clone `https://github.com/archimedes/omni-harness.git` if needed, then change into the repository root.
...
5. Exact next command to start OmniHarness

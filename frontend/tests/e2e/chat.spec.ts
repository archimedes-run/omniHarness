...
test.describe("Chat workspace", () => {
  ...
  test("can type a message in the input box", async ({ page }) => {
    ...
    await textarea.fill("Hello, OmniHarness!");
    await expect(textarea).toHaveValue("Hello, OmniHarness!");
  });
  ...
  test("sending a message triggers API call and shows response", async ({
    ...
    await expect(page.getByText("Hello from OmniHarness!")).toBeVisible({
      timeout: 10_000,
    });
  });
});

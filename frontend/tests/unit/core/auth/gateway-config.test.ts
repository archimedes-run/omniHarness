...
const ENV_KEYS = [
  "NODE_ENV",
  "OMNI_HARNESS_INTERNAL_GATEWAY_BASE_URL",
  "OMNI_HARNESS_TRUSTED_ORIGINS",
] as const;
...
describe("getGatewayConfig", () => {
  ...
  beforeEach(() => {
    saved = snapshotEnv();
    setEnv("OMNI_HARNESS_INTERNAL_GATEWAY_BASE_URL", undefined);
    setEnv("OMNI_HARNESS_TRUSTED_ORIGINS", undefined);
  });
  ...
  test("uses env values verbatim when set, regardless of NODE_ENV", async () => {
    setEnv("NODE_ENV", "production");
    setEnv("OMNI_HARNESS_INTERNAL_GATEWAY_BASE_URL", "https://gw.example.com/");
    setEnv(
      "OMNI_HARNESS_TRUSTED_ORIGINS",

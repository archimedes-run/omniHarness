...
description: This page covers day-to-day operational tasks and solutions to common problems when running OmniHarness App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running OmniHarness App.
...
| File                 | Service                              |
| -------------------- | ------------------------------------ |
| `logs/langgraph.log` | LangGraph / OmniHarness Harness runtime |
...
When you pull a new version of OmniHarness, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
- Check that the storage path is writable: `ls -la backend/.omni-harness/`.
- Look for memory update errors in `logs/langgraph.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.omni-harness/`:

```
backend/.omni-harness/
...
In Docker deployments, the bind-mounted host path (`$OMNI_HARNESS_ROOT/backend/.omni-harness/`) is the source of truth — back up the host path.

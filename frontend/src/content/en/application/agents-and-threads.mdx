...
description: OmniHarness App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
---
...
OmniHarness App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
...
<Callout type="info">
  ...
  embedded <code>OmniHarnessClient</code> (used in direct Python integrations), not
  to the LangGraph Server deployment used by OmniHarness App.
</Callout>
...
```
backend/.omni-harness/threads/{thread_id}/user-data/
```

This directory is mounted into the sandbox as `/mnt/user-data/` for the agent to read and write.

In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `OMNI_HARNESS_ROOT` to the absolute path of your omni-harness repository root to ensure the correct host path is used.

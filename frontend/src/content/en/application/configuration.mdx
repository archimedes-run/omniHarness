---
title: Configuration
description: OmniHarness App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
OmniHarness App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
Install Ollama provider: `cd backend && uv add 'omniharness-harness[ollama]'`
...
Install: `cd backend && uv add 'omniharness-harness[aio-sandbox]'`
...
tools:
  ...
  - use: omniharness.community.ddg_search.tools:web_search_tool # default, no key required
  ...
  - use: omniharness.sandbox.tools:read_file_tool
  - use: omniharness.sandbox.tools:glob_tool
  - use: omniharness.sandbox.tools:grep_tool
  - use: omniharness.sandbox.tools:write_file_tool
  - use: omniharness.sandbox.tools:str_replace_tool
  - use: omniharness.sandbox.tools:bash_tool
```

### Thread state persistence (checkpointer)

By default, OmniHarness uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.omni-harness/
```
...
checkpointer:
  type: postgres
  connection_string: postgresql://user:password@localhost:5432/omniharness
```

---
title: Configuration
description: "OmniHarness's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment."
---
...
<Callout type="info" emoji="⚙️">
  All OmniHarness Harness behaviors are driven by <code>config.yaml</code>. One
  file controls which models are available, how the sandbox runs, what tools are
  loaded, and how each subsystem behaves.
</Callout>

OmniHarness's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment.

## Config file location

OmniHarness resolves `config.yaml` using the following priority order:

1. The path passed to `AppConfig.from_file(config_path)` explicitly.
2. The `OMNI_HARNESS_CONFIG_PATH` environment variable.
...
```bash
export OMNI_HARNESS_CONFIG_PATH=/path/to/my-config.yaml
```
...
This pattern is how OmniHarness achieves pluggability without hardcoding class references.

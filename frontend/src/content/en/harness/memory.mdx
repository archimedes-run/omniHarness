---
title: Memory
description: Memory is a runtime feature of the OmniHarness Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets OmniHarness carry useful information across sessions. The agent
...
Memory is a runtime feature of the OmniHarness Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  # Default: {base_dir}/memory.json  (resolves to backend/.omni-harness/memory.json)
  ...
  storage_class: omniharness.agents.memory.storage.FileMemoryStorage
...
OmniHarness supports two levels of memory:
...
- Base directory: `backend/.omni-harness/`
- Global memory: `backend/.omni-harness/memory.json`
- Per-agent memory: `backend/.omni-harness/agents/{agent_name}/memory.json`

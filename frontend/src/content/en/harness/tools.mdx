...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. OmniHarness provides built-in
...
OmniHarness organizes tools into four categories:
...
tools:
  ...
  - use: omniharness.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
Install: `cd backend && uv add 'omniharness-harness[tavily]'`
...
Install: `cd backend && uv add 'omniharness-harness[exa]'`
...
Install: `cd backend && uv add 'omniharness-harness[firecrawl]'`
...
tools:
  ...
  # Or use InfoQuest:
  # - use: omniharness.community.infoquest.tools:image_search_tool
  #   api_key: $INFOQUEST_API_KEY
```

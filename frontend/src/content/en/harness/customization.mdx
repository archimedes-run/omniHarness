---
title: Customization
description: OmniHarness's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  OmniHarness is designed to be adapted. You can extend agent behavior by writing
...
OmniHarness's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
```python
from omniharness.agents.memory.storage import MemoryStorage
...
For custom backends, implement the LangGraph `BaseCheckpointSaver` interface and configure it programmatically when initializing the `OmniHarnessClient`.

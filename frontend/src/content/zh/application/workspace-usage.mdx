---
title: 工作区使用
description: OmniHarness 工作区是一个基于浏览器的对话界面，你可以在其中向 Agent 发送消息、上传文件、查看中间步骤，以及下载生成的产出物。
---
...
<Callout type="info" emoji="💬">
  OmniHarness 工作区是你与 Agent
  交互的地方。本页面涵盖主要用户界面工作流——创建对话、上传文件、查看产出物和使用技能。
</Callout>

OmniHarness 工作区是一个基于浏览器的对话界面，你可以在其中向 Agent 发送消息、上传文件、查看中间步骤，以及下载生成的产出物。
...
产出物会持久保存在线程的用户数据目录中（`.omni-harness/threads/{thread_id}/user-data/outputs/`）。

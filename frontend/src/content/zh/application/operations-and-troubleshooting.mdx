---
title: 运维与排障
description: 本页面涵盖运行 OmniHarness 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 OmniHarness 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

OmniHarness 应用在 `logs/` 目录中写入每个服务的日志：
...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.omni-harness/threads/
```

**解决**：

1. 确保 `backend/.omni-harness/` 对运行 OmniHarness 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`OMNI_HARNESS_ROOT` 设置为绝对路径）。
...
OmniHarness 将持久化数据存储在：

- **线程数据**：`backend/.omni-harness/threads/` — 每个线程的上传文件、输出和工作区文件
- **检查点**：取决于检查点器配置（SQLite：`backend/.omni-harness/checkpoints.db`，Redis：外部存储）
- **记忆**：`backend/.omni-harness/memory.json`（以及 `agents/*/memory.json`）

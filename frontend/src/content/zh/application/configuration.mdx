---
title: 配置
description: 本页面涵盖 OmniHarness 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。
---
...
本页面涵盖 OmniHarness 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。

## config.yaml

`config.yaml` 是 OmniHarness 的主要配置文件。所有 Agent 行为、模型选择、工具加载和运行时功能都由它控制。
...
checkpointer:
  type: sqlite
  connection_string: .omni-harness/checkpoints.db
...
技能启用状态会反映在 `extensions_config.json` 中。你可以直接编辑它，或通过 OmniHarness 应用界面进行管理。

## 运行时环境变量

这些变量在 OmniHarness 进程中设置（通过 `.env`、Docker 环境或 shell）：

| 变量                    | 默认值           | 描述                                             |
| ----------------------- | ---------------- | ------------------------------------------------ |
| `OMNI_HARNESS_CONFIG_PATH` | 自动发现         | `config.yaml` 的绝对路径                         |
| `LOG_LEVEL`             | `info`           | 日志详细程度（`debug`/`info`/`warning`/`error`） |
| `OMNI_HARNESS_ROOT`        | 仓库根目录       | 用于 Docker 中的技能和线程挂载                   |

...
description: OmniHarness 应用是 OmniHarness 生产体验的参考实现。它将 Harness 运行时、基于 Web 的对话工作区、API Gateway 和反向代理组合成一个可部署的完整系统。
---
...
<Callout type="info" emoji="🚀">
  OmniHarness 应用是构建在 OmniHarness Harness 之上的完整 Super Agent
  应用。它将运行时能力打包成一个可部署的产品，包含 Web 界面、API Gateway
  和运维工具。
</Callout>

OmniHarness 应用是 OmniHarness 生产体验的参考实现。它将 Harness 运行时、基于 Web 的对话工作区、API Gateway 和反向代理组合成一个可部署的完整系统。
...
OmniHarness 应用以四个服务的形式运行，通过单个 nginx 反向代理提供：
...
- **LangGraph**：OmniHarness Harness 运行时，管理线程状态、Agent 执行和流式传输。
...
| Agent 运行时 | LangGraph、LangChain、OmniHarness Harness                  |
| 反向代理     | nginx                                                   |

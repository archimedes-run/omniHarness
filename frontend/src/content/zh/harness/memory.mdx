---
title: 记忆系统
description: 记忆是 OmniHarness Harness 的一个运行时功能。它不是简单的对话日志，而是跨多个独立会话持久化、在未来对话中影响 Agent 行为的结构化事实和上下文摘要存储。
---
...
<Callout type="info" emoji="💾">
  记忆让 OmniHarness 在多个会话中保留有用信息。Agent
  记住用户偏好、项目背景和反复出现的事实，这样它可以在不每次从零开始的情况下给出更好的响应。
</Callout>

记忆是 OmniHarness Harness 的一个运行时功能。它不是简单的对话日志，而是跨多个独立会话持久化、在未来对话中影响 Agent 行为的结构化事实和上下文摘要存储。
...
memory:
  ...
  # 全局记忆文件的存储路径。
  # 默认：{base_dir}/memory.json（解析为 backend/.omni-harness/memory.json）
...
- 基础目录：`backend/.omni-harness/`
- 全局记忆：`backend/.omni-harness/memory.json`
- 按 Agent 记忆：`backend/.omni-harness/agents/{agent_name}/memory.json`

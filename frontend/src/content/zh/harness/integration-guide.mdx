---
title: 集成指南
description: OmniHarness Harness 不仅仅是一个独立应用程序——它是一个可以导入并在你自己的后端、API 服务器、自动化系统或多 Agent 协调器中使用的 Python 库。
---
...
<Callout type="info" emoji="🔌">
  OmniHarness Harness 可以嵌入任何 Python 应用程序。本指南涵盖在你自己的系统中将
  OmniHarness 作为库使用的集成模式。
</Callout>

OmniHarness Harness 不仅仅是一个独立应用程序——它是一个可以导入并在你自己的后端、API 服务器、自动化系统或多 Agent 协调器中使用的 Python 库。

## 嵌入 OmniHarnessClient

主要集成点是 `OmniHarnessClient`。它封装了 LangGraph 运行时，并提供一个简洁的 API，用于在任何 Python 应用中发送消息和流式传输响应。

```python
from omniharness.client import OmniHarnessClient
from omniharness.config import load_config

# 加载配置（读取 config.yaml 或 OMNI_HARNESS_CONFIG_PATH）
load_config()
...
OmniHarness Gateway 本身是一个 FastAPI 应用程序。你可以将其作为子应用程序挂载：

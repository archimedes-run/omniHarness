...
<Callout type="info" emoji="📦">
  沙箱是 Agent 进行文件和命令操作的隔离工作区。它让 OmniHarness
  能够采取真实行动，而不仅仅是对话。
</Callout>
...
OmniHarness 支持三种沙箱模式，选择适合你部署的一种：
...
安装：`cd backend && uv add 'omniharness-harness[aio-sandbox]'`
...
| `.omni-harness/threads/{thread_id}/user-data/` | `/mnt/user-data/`                             | 读写     |

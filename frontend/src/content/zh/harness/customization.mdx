---
title: 自定义与扩展
description: OmniHarness 的可插拔架构意味着系统的大多数部分都可以在不 fork 核心的情况下被替换或扩展。本页面列举了扩展点，并解释如何使用每一个。
---
...
OmniHarness 的可插拔架构意味着系统的大多数部分都可以在不 fork 核心的情况下被替换或扩展。本页面列举了扩展点，并解释如何使用每一个。
...
对于自定义后端，实现 LangGraph 的 `BaseCheckpointSaver` 接口，并在初始化 `OmniHarnessClient` 时以编程方式配置它。
